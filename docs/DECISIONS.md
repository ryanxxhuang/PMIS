...
- **狀態**：ACCEPTED（2026-08-13）
- **決策**：公共工程主品牌採 `GovAgent｜公共工程`；側欄固定為「今日待辦、現場與品質、審查與協作、進度與金流、文件與結案、專案」六個工作面；`問 GovAgent` 是全域入口，不是第七個模組。廠商／監造預設進今日待辦，機關預設進跨案總覽。
- **結果**：W8-1 已由 PR #11 交付品牌、六面導覽、共同頁首、角色落地、手機抽屜與手機工作面選單；36 條路由、原三角色權限、RLS 與資料庫均未改動。W8-2～W8-5 依核准報告逐包接續，不得把 UI 改版擴張成後端重寫。
