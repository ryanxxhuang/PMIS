...
- **IA 殼**：**落地點已完成**——側欄有「球在誰手上」來源群組（現在輪到我／等待對方／今天已完成），主畫面依 `?ball=` 一次聚焦一段，四張指標卡已依判準第 2 條移除。**清單／詳情殼已推廣到七頁**：契約重點、擷取審核、工安管理、工程疑義、期限追蹤、送審文件、變更設計。桌機常駐右欄、`<lg` 走抽屜。原文高亮（方向 C 的詳情呈現）仍待整合。
