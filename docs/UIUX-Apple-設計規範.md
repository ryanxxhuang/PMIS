...
### 9.6 表格頁在手機＝唯讀摘要
- `/valuation`（1040px 標單樹）、`/cost`（520/760）、`/schedule`（720）、`/boq`（640）、`/monthly-report`（433）在 <md 隱藏寬表，改渲染卡片／時間線摘要（照 `/payments` 前例），並明講「編輯在桌面進行」。**決策：手機不提供這五頁的寫入。**（`/cost` 自 P1b 起桌機也無寫入，手機摘要不再需要「編輯在桌面」那句。）
