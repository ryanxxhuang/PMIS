...
本工作包依使用者本次授權調整導覽呈現：手機三格改為各角色常用的具名子頁；桌機首頁提供精簡的「常用工作」操作列（不放摘要卡），手機由底欄承接，不重複顯示；完整待辦留在首頁，不再截斷後跳提醒中心。功能尋找只搜尋已授權的頁面名稱及工作說明，不依賴 AI。保留既有階層導覽、色票與手機唯讀界線。手機清單的搜尋／類型／期限篩選預設收合，按「篩選」展開；有篩選參數時自動展開。首頁每次顯示 20 件，可原地顯示更多；返回單據前的篩選與載入筆數保存在 URL。詳情抽屜可直接返回今日待辦。各工作頁依角色顯示操作提示，契約原文優先與估驗金額說明保留。
...
- BottomNav 5 格 ＝ 現在輪到我 ＋ 三個角色常用子頁 ＋ **「更多」**（iOS tab bar 上限 5）。「更多」開既有的導覽抽屜；頂欄漢堡在 <md 退場——同一份抽屜兩個入口是稽核點名的「兩層平行導覽」。抽屜的 e2e 合約（`navigation[name=主要功能]`、開啟聚焦關閉鈕、Esc 關閉並還焦點給**觸發它的鈕**）不變，觸發鈕改由「更多」承接：accessible name 就是可見文字「更多」（WCAG 2.5.3 label-in-name，不用 aria-label 蓋成別的字），`aria-expanded` 綁抽屜開合，e2e 改點「更多」、焦點回「更多」。手機頂欄只留品牌、專案切換、AI 助理鈕、提醒；主題切換與登出移到抽屜底部的模式列（稽核量到頂欄 434 > 375）。
