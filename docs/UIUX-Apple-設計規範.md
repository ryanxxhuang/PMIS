...
### 三方操作流程優化工作包（2026-09-14）

問題：三方共用相同捷徑、首頁五筆截斷後轉往另一清單、待辦深連結與返回動線不完整，功能名稱難以對應實際工作。
目標：維持收件匣落地，讓使用者從角色常用工作或完整待辦直達單據，知道自己的責任並可返回原清單。
不做：不改業務角色、核定權責、資料模型、計算與後端授權；不另建 dashboard。
影響：首頁、全域功能尋找、手機捷徑、三方操作說明、待辦深連結與共用詳情版面。
驗收：三方常用入口正確；首頁原地搜尋／篩選／看更多；待辦直達指定單據並返回原篩選；手機／平板可達且不溢位；既有測試與文件檢查通過。

本工作包依使用者本次授權調整導覽呈現：手機三格改為各角色常用的具名子頁；桌機首頁提供精簡的「常用工作」操作列（不放摘要卡），手機由底欄承接，不重複顯示（**此兩項已由下方 D-026 四主入口取代**）；完整待辦留在首頁，不再截斷後跳提醒中心。功能尋找只搜尋已授權的頁面名稱及工作說明，不依賴 AI。保留既有階層導覽、色票與手機唯讀界線。手機清單的搜尋／類型／期限篩選預設收合，按「篩選」展開；有篩選參數時自動展開。首頁每次顯示 20 件，可原地顯示更多；返回單據前的篩選與載入筆數保存在 URL。詳情抽屜可直接返回今日工作。各工作頁依角色顯示操作提示，契約原文優先與估驗金額說明保留。
...
| **同組子頁導航**（2026-09-15 入口方案 B） | 側欄是同組子頁的主要入口：進頁或切換路由時自動展開目前頁所屬群組並列出子頁，使用者可手動收合／展開。內容區的分頁列（`PageTabs`）只在側欄提供不了同組導航時出現：側欄收合成 icon rail、平板 768–1279、手機抽屜、或使用者手動收合該組。這是 D-015「內容區不重複子頁導覽」的解讀：側欄已顯示子頁時不重複；側欄顯示不了時內容區得替代顯示，子頁入口永遠不消失。 | 階段 0 §3 |
| **四主入口**（2026-09-17 D-026，瘦身 P1a／P1c） | 主入口只有四個：今日工作（球權三來源，持有 `/dashboard`）、現場紀錄 `/site`、履約時程 `/requirements`、估驗請款 `/valuation`。側欄分區＝今日工作 → 工作（三主入口群組＋子頁）→ 專案資料（次入口：文件往來／專案兩組）→ 平台。三角色的主入口相同，角色差異只在頁內依角色列出的作業；桌機首頁的主入口列與側欄「工作」分區同名同內容（`WORK_TITLE`），手機五格底欄＝現在輪到我＋三主入口＋更多。頁面文案指路一律取 `navLabel`／`navEntryFor` 的名字，不手抄。退場頁（成本管理、風險稽核、逐工項排程）`hidden` 且唯讀（P1b：成本寫入由資料庫收回、只剩歷史查閱與 CSV；風險稽核的估驗勾稽檢核移入估驗計價頁逐期顯示，頁面只剩唯讀查閱與導向；P5d：關鍵工項的計畫起迄承接到履約時程的同一條時間軸維護，排程頁只剩歷史查閱與 CSV），深連結與 roles 不變；跨案總覽縮為選案清單。`/dashboard` 的 h1 與側欄分區同名「今日工作」（`BALL_SOURCES_TITLE`），不再有「今日待辦」別名。 | [D-026](DECISIONS.md)、[入口與退場](architecture/slimming-entrypoints-and-retirement.md) §1 |
...
- 點擊可達性：三角色從側欄用點的走到 `visibleNavGroups` 每個子頁、球權三來源，URL 與 `aria-current` 都對 — `reachability.spec.js`。期望值直接算自 `navConfig.js`，不手抄。首頁主入口列與底欄＝`roleWorkLinks`，名稱＝`WORK_TITLE` — `workflow-ux.spec.js`。
- `nav` 收合時顯示短標（navConfig 各項的 `short`，與 label 同住；舊 `NAV_SHORT` 對照表已退場）。
- 手機抽屜與斷點：375 開抽屜後拉寬到 1024，`body` 鎖定必須隨抽屜消失而解除、捲動位置還原；縮回 375 抽屜不自己彈開 — `a11y.spec.js`。
...
- **IA 殼**：**三個方向都已落地**。落地點（方向 A）——側欄第一區「球在誰手上」（現在輪到我／等待對方／今天已完成）持有 `/dashboard`，主畫面依 `?ball=` 一次聚焦一段，四張指標卡已依判準第 2 條移除。**導覽已完成從「工作面」到來源模型的遷移（2026-09-11）**：側欄分區＝球在誰手上 → 工作（現場與品質／審查與協作／進度與金流／報表與結案／專案，群組＋子頁，子頁依角色過濾、整組不可見就不渲染）→ 參考（契約重點／專案文件／標單工項，扁平項）→ 平台（僅平台管理員）。「今日待辦」工作項退場，`/dashboard` 只剩球權來源一個入口，`aria-current` 天然只落一處；五個群組解封，Dashboard 的「最近施工日誌」卡隨之移除（它的存在理由只是可發現性）。手機 BottomNav＝主畫面槽（現在輪到我）＋三個角色常用子頁＋更多（2026-09-14 工作包）。**2026-09-17 依 D-026 再分區為四主入口**（§0 表末列）：側欄＝今日工作 → 工作（現場紀錄／履約時程／估驗請款）→ 專案資料（文件往來／專案）→ 平台，參考分區與角色常用子頁退場；底欄與首頁主入口列改為三個主入口群組，`/cost`、`/audit` hidden。**清單／詳情殼已推廣到十四頁**（方向 B）：契約重點、擷取審核、工安管理、工程疑義、期限追蹤、送審文件、變更設計、檢驗停留點、提醒中心、三方成員、活動紀錄、風險稽核、品質查驗（查驗／試體／觀察三段各自套殼，路由器本身不套）、專案文件。桌機常駐右欄、`<lg` 走抽屜。**原文高亮（方向 C）已完成**：`f4fbfca` 原文定位器（引述比對回原文位置）、`0c0b4f8` 契約重點詳情欄顯示契約原文＋條文高亮。
  ...
  - **不套殼的四頁**：`/quality`（只是路由器，清單在子元件）、`/valuation`（1040px 標單樹）、`/payments`（10 欄就地編輯表，每格本來就可編）、`/cost`（退場後為唯讀帳冊：統計卡＋分類表＋明細表，只借 `SearchField`＋分類快篩 chip 與 CSV 匯出，不再就地編輯）；`ChecklistSection`（自檢表範本編輯器，只借快篩與清單語意）與 `/site-log`、`/supervisor-log`（整日日誌編輯器；監造日誌版面由伺服器範本 sections 驅動）同理。它們沿用殼的零件即可，不要硬套版面。
...
- BottomNav 5 格 ＝ 現在輪到我 ＋ 三個主入口群組（現場紀錄／履約時程／估驗請款，2026-09-17 D-026；原為角色常用子頁）＋ **「更多」**（iOS tab bar 上限 5），群組在任一子頁都算選取。「更多」開既有的導覽抽屜；頂欄漢堡在 <md 退場——同一份抽屜兩個入口是稽核點名的「兩層平行導覽」。抽屜只存在於 <md：開合、鎖捲動、Esc、遮罩、焦點全由 `isBelowMd && menuOpen` 推導，拉寬到 ≥md 抽屜與鎖定同時消失、開啟意圖清除（先前只讀 `menuOpen`，拉寬後 body 仍固定在 `top:-scrollY`、內容鎖在畫面外）。抽屜的 e2e 合約（`navigation[name=主要功能]`、開啟聚焦關閉鈕、Esc 關閉並還焦點給**觸發它的鈕**）不變，觸發鈕改由「更多」承接：accessible name 就是可見文字「更多」（WCAG 2.5.3 label-in-name，不用 aria-label 蓋成別的字），`aria-expanded` 綁抽屜開合，e2e 改點「更多」、焦點回「更多」。手機頂欄只留品牌、專案切換、AI 助理鈕、提醒；主題切換與登出移到抽屜底部的模式列（稽核量到頂欄 434 > 375）。
...
### 9.6 表格頁在手機＝唯讀摘要
- `/valuation`（1040px 標單樹）、`/cost`（520/760）、`/boq`（640）、`/monthly-report`（433）在 <md 隱藏寬表（`/schedule` 的 720px 表隨頁面於 P6b 移除），改渲染卡片／時間線摘要（照 `/payments` 前例），並明講「編輯在桌面進行」。**決策：手機不提供這五頁的寫入。**（`/cost` 自 P1b、`/schedule` 自 P5d 起桌機也無寫入，手機摘要不再需要「編輯在桌面」那句；關鍵工項的計畫起迄改在 `/requirements` 事項詳情維護，加入關鍵工項的搜尋只在桌機。）
...
- 做法：`todayTasks` 的 `to` 帶單條 query（七個殼頁的 `useListDetailPane` 已支援：`/rfi?rfi=`、`/submittals?submittal=`、`/change-orders?co=`、`/deadlines?obligation=`、`/requirements?obligation=`、`/requirements/review?highlight=`、`/safety?record=`）；<lg 深連結進入時直接開抽屜（`select(id, { openPane: true })`）。2026-09-14 補齊品質查驗／觀察／試體、ITP 的單條深連結；估驗與請款用 `?period=` 選定期別。驗收與日誌維持頁面入口。
