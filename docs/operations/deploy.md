...
- 新表要檢查 grants：基線的 `alter default privileges` 會讓新表自動帶 authenticated 的 SELECT／INSERT／UPDATE／DELETE，migration 內要明確 `revoke`／收窄（`DEVELOPMENT.md` §5）。TRUNCATE／REFERENCES／TRIGGER／MAINTAIN 自 `20260917213900` 起已從 anon／authenticated／service_role 與 default privileges 收回，新表不會再自動帶，pgTAP `api_roles_table_privileges.sql` 對全部 public 表迴圈斷言；表必須由 `postgres`（migration）建立，default privileges 才會生效。自 `20260919003000` 起 anon 對 public 表／序列／函式一律零權限（新物件不再自動帶），新函式對 anon／authenticated／PUBLIC 都不可執行：給前端／Edge user client 用的 RPC 要在 migration 明示 `grant execute … to authenticated` 並加進 pgTAP `anon_and_function_privileges.sql` 的允許清單，否則 pgTAP 紅、上線後 42501（fail-closed）；service_role 維持平台預設可執行。
