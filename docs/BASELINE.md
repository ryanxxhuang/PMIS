...
| pgTAP | **40 檔 1018 條斷言**全過 | 本機 `supabase db reset`（從零套用全部 migration）後照 [`../supabase/SETUP.md`](../supabase/SETUP.md) 跑 `supabase/tests/*.sql`；CI 跑法見 `.github/workflows/pgtap.yml` | 靜態交叉核對：`grep -ho "plan([0-9]*)" supabase/tests/*.sql`（`plan()` 加總應等於實跑斷言數） |
...
| 項目 | 數字 | 指令 |
|---|---|---|
| migrations | **59** 支；最新 `20260911100100` | `ls supabase/migrations/*.sql \| wc -l` |
| 其中尚未套用正式庫 | **2** 支：`20260911100000_demo_requests_revoke_grants`、`20260911100100_contract_parse_retire` | `supabase migration list --linked`（動 DB 前必看） |
| rollbacks（down 檔） | **9** 支（未演練回復） | `ls supabase/rollbacks/*.sql \| wc -l` |
| pgTAP 檔 | **40** | `ls supabase/tests/*.sql \| wc -l` |
...
| 項目 | 最後核對 | 狀態 |
|---|---|---|
| 正式庫 migration tracker | 2026-09-02（`supabase migration list --linked`） | 當時本機 57 支＝遠端 57 筆，遠端最新 `20260901040000`。之後新增 §2 的兩支**尚未套用**；未重核 |
...
| CI 與分支保護 | 2026-09-07 | `main` 有 active ruleset（需 PR、`test-and-build`、`pgtap`）；pgTAP 自 PR #62 起每個 PR 一律跑；仍允許 RepositoryRole 5 bypass、未要求核准人數與分支先更新 |
...
| pgTAP | 33 檔／`plan()` 加總 927（當時靜態統計） | 40 檔 1018（實跑） |
