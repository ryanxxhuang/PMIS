...
> ACTIVE｜2026-09-21｜Demo 急件表內上傳與三核心導覽（Vitest＋pgTAP＋真後端 live 五張真照片）、P5e 保固類循環義務的停止條件（pgTAP 全套＋rollback 實跑＋Vitest 前端／Edge 路徑＋Deno 執行期＋Demo E2E 6 支＋真後端 chain 15／3＋內建 Preview 375／1024）、P3f 捨棄現場文書草稿（pgTAP＋Vitest＋Demo E2E＋真後端 chain 14 與 chain 5／6／8／10／12 回歸）、P6b-3 品質直接寫入退場（pgTAP 全套＋真後端 chain 4／8／10／13＋rollback 實跑）、P6b-2 Agent 草稿改接文件（Vitest＋Demo E2E 全套＋本機共用棧一次性真 DB 驗證）、P6b-1 移除退場頁與退場 Edge 原始碼（Vitest＋Demo E2E 5 支）、P6a 月報／監造月報／佐證包只重用已簽署資料（Vitest＋Demo E2E＋真後端 chain 13＋內建 Preview 375／1024）、P3e 共用補值（pgTAP＋Vitest＋真後端 chain 12 與 chain 5／6／8／10 回歸）、P4e 收回估驗明細直接寫入（pgTAP 全套＋rollback 實跑＋真後端 chain 2／7／9／11）、O1 demo 站重佈與 P3c 合併結果同步（`check:prod` 五頁＋正式庫唯讀核對）、T2 Demo E2E 受測 dev server 隔離（修正前後對照＋Demo E2E 全套）、T1 真後端 E2E 受測 dev server 不監看檔案（chain 5→6→7 連跑 3 輪）、P4c 估驗頁接上後端強制（Vitest＋Demo E2E＋真後端 chain 2／7／9＋內建 Preview 375／1024）、P3d 施工日誌列印與提送回執／退回歷史呈現（Vitest＋Demo E2E＋真後端 chain 5／6／8＋內建 Preview 375／1024）、P3b 自主檢查表、P4b 監造確認量後端強制（pgTAP＋dblink 真併發＋Vitest＋真後端 chain 2／7）、P6c `audit.summary` 退場（pgTAP＋Vitest＋Deno＋Demo E2E）、CI1 釘住 pgTAP workflow 的 Supabase CLI 版本（本機 pgTAP＋PR CI 實跑）、P5d 履約時程 UI（Vitest＋Demo E2E＋內建 Preview 1024／375）、R1 全面移除兩步驟驗證（pgTAP＋Vitest＋Demo E2E＋真後端 chain 1／5／6）、P5c 基準日版本（pgTAP＋Vitest 前端／Edge 路徑＋Deno 執行期＋Demo E2E＋內建 Preview）、D1 正式站邊緣注入與 `check:prod` 漏檢（Vitest＋`wrangler dev`＋demo 重佈實測）、P3a 監造日誌頁面（Vitest＋Demo E2E＋真後端 chain 6）、H2／H3 anon 與函式 EXECUTE 權限硬化（pgTAP 全庫迴圈＋真後端 E2E 四鏈）、P5b 循環義務逐期追蹤（pgTAP＋Vitest 前端／Edge 路徑＋Deno 執行期＋Demo E2E）、P3a 監造日誌後端（pgTAP＋Vitest＋Deno）、P5a 球權單一實作與共用案例（Vitest 前端／Edge 路徑＋Deno 執行期＋pgTAP）、H1 表級權限硬化（pgTAP 全表迴圈＋真後端 E2E）、P2b Edge 起稿（Vitest＋pgTAP＋Deno）、瘦身 P1b 退場頁唯讀化（成本寫入 DB 收回 pgTAP）、P2d 施工日誌存版／簽署／提送 RPC pgTAP、瘦身 P1c／P1d 文案對齊與手機抽屜斷點缺陷、P2a 現場文書資料層 pgTAP、T0 本機 pgTAP 隔離、P4a 純計算層 pgTAP；保留 2026-09-14 三方 UIUX 與 2026-09-12 的既有後端與全案驗證快照。
> 手動實跑快照，不是 CI 自動產物。前一版驗證紀錄可從 Git 追溯；正式環境狀態只見 [CURRENT §6.3](../CURRENT.md#63-正式環境最後核對不是即時狀態)。

## 1. 本輪驗證

### 2026-09-21 Demo 急件：表內上傳 AI 填表、廠商三核心導覽、G 包自檢表兩向尺寸讀數（PR #177，merge `528d3fc`；migration `20260921030000`；Opus 5 主控、表內上傳由 Fable 5.1 實作；使用者指定的 opus5.2 不存在，低風險部分由 Opus 5 執行）

- `npm test` 164 檔 1,875 項；`npm run test:edge` 27；`check:edge`；`lint` 零警告；`build`；`check:docs` 0 錯；`npm run test:db`（一次性 DB 從零套 89 支）65 檔 3,739 通過。
- Demo E2E：導覽相關 5 支（a11y／contractor／routes／workflow-ux／reachability）照三核心新規格更新後 30＋項通過；PR CI e2e 全套綠。
- 真後端（本機棧，`functions serve` stub＋本機 CRON 密鑰）：全套 29 項＝25 通過、2 依設計略過（live 鏈）、chain21 紅（/site 總覽頁廠商上傳選檔後未開始，連跑 3 次同樣；Fable 單獨跑時曾綠；該入口對廠商已收起）。auth-smoke 照新落地頁更新後通過。
- 真後端 live（真模型、`.env.e2e.real` 金鑰未輸出）：chain25 五張真實照片表內上傳——施工日誌 5 次 `photo.classify`、約 42 秒填入 3 欄（天氣上午「晴」取自紙表、施工概況、工項列），修改→存檔→重整→PDF；自檢表同批照片 0 次新模型呼叫、約 0.8 秒填入工項與兩項讀數：線徑 編號 1 13×11、編號 4 11×11 mm，網目 編號 1、4 皆 15×15 cm（4/4 命中人工標註），搭接與勾選項留待補，存檔→重整→PDF 印出讀數。chain3 live 第一次模型輸出不完整、第二次通過；chain7／9 通過。報告檔 `test-results/live-inform-real-photos.json`（含真實照片辨識內容，不進版控）。
- 部署後：`check:prod` 五頁 OK；正式入口 chunk 含新按鈕與「先存檔再下載」。
...
- `npm run test:db`（一次性資料庫從零套全部 migration＋seed；rebase 到含 P6b-2／P6b-3／P3f 的 main `1ee8f96`、migration 時間戳改為 `20260920040000` 後重跑）：60 檔、3,521 通過、0 失敗（rebase 前 58 檔 3,386；差額為 P6b-3 `quality_direct_writes_retired.sql` 50、P3f `field_document_discard.sql` 78 與允許清單同步）。新增 `warranty_stop_condition.sql` 105 條：結構與授權（RPC 成員／service 可、anon 不可、內部函式不開放）；保固期滿日日期規則 10 例（與共用 fixture `warranty.expiry_cases` 同一組：2025-04-30＋1 個月＝05-31、2023-02-28＋1 年＝2024-02-29、2025-01-30＋1 個月＝02-28、閏年／平年 1-28、跨年月底、以日計）與兩個非台北 session 時區結果不變；缺口文字四種；兩項皆缺／只缺期間 → 不產生；guard（非管理者、待確認條文、別案條文、有期間無條文、有條文無期間、未知單位、非整數、0、直接 REST 引用待確認條文皆拒）；登錄 1 年 → 留版（`changed_keys` warranty_term、`warranty` 快照、四日期快照形狀不變）、期次只在保固期間（起於合格日後第一個到期日、止於期滿日所在期、不看開工觸發點）、basis 記合格日／期滿日／期間／引用條文、非保固類不受影響、每日推進不越界；動過兩期後更正為 6 個月 → 只移除沒動過的期並記 removed／kept；合格日更正延後 → 早於新合格日的待辦期移除、期滿延後補回、已完成期保留原依據；改不合格 → 只留動過的期、改回合格冪等補回；引用條文被取代 → 停止計算並移除未動過的期、改引用新條文（直接 REST，也留版）→ 恢復；類別保固↔非保固 → 重建未動過的期；引用條文刪除（FK set null）→ 期間保留、停止計算、留版；有保固期間的專案可整案刪除。`project_anchor_versions.sql` 124（三條保固斷言改新語意）、`anon_and_function_privileges.sql` 418（允許清單 83 支）。
...
- `npm test`：143 檔、1,560 項（rebase 到含 P6b-2／P6b-3／P3f 的 main 後；rebase 前 142 檔 1,566，差異來自 P6b-3／P3f 的測試改動）。新增 `components/WarrantyBasis.test.jsx` 5、共用案例前端路徑＋3（pgTAP 覆蓋同一組日期案例與缺口文字、五種保固事實的待補設定與入口）、Edge 路徑＋6（五種保固事實、收集器讀 RPC 帶 `need`）、`obligationTimeline.test.js`＋3（期程段保固期、版本紀錄保固期間變更、基準日缺口不含保固類）、`projects.anchors.test.js`＋3、`db.test.js`＋2、`ledger.test.js`＋1（驗收登錄／撤銷後重載期次與保固事實）；`todayTasks`／`obligationTimeline` 保固斷言改新語意；Agent 唯讀 RPC 白名單掃描改三支。
- `npm run test:edge` 5（Deno 執行期同一組保固案例）；`npm run check:edge`；`npm run lint` 零警告；`npm run build`。
- Demo E2E（自起 5188、不設 `CI`；rebase 後重跑）：contractor／owner／supervisor／contract-flow／a11y／workflow-ux 6 支 66 項通過。
- 真後端 E2E（共用開發 DB 以 psql 套本支並登記版本，rebase 後版本號同步改為 `20260920040000`；無模型金鑰的固定資料模式；rebase 後重跑兩鏈皆通過）：新增 chain 15——廠商（管理者）補登兩條保固契約重點、監造確認 → 期程卡「缺正式驗收合格日、缺契約保固期間」、保固巡檢無期次；管理者在期程卡登錄 2 年並引用條文 → 留版、仍不產生；機關登錄正式驗收合格 2025-03-15 → RPC 期滿 2027-03-15、期次起於 2025-04 且全部 ≤ 期滿日；監造看到同一份依據、無登錄按鈕、期次依據句寫出合格日與期滿日。chain 3 通過（「擷取審核」選擇器限定主內容區，側欄同名子頁造成的既有 strict mode 衝突）。
- 內建 Preview（demo，自起 5196）：履約期程卡「保固期滿日待補（缺正式驗收合格日…）」與兩項依據，1024／375 無水平溢位。
- 合併後（merge commit `4d6d77b`；migration 由使用者於 2026-09-20 `db push` 套用，與 P6b-3 `20260920030000` 同批）：Workers Builds success、正式入口 chunk `index-But1e0G2.js` 含 `get_project_warranty`；四支 Edge 以 `--use-api` 重佈（`agent-run` v21、`send-reminders` v23 且 `verify_jwt=false` 維持、`fetch-weather` v19、`draft-field-documents` v10）；`node scripts/check-prod.js` 五頁 OK；正式庫唯讀核對：RPC 對 `authenticated` 可執行／`anon` 不可、三欄與版本表 `warranty` 欄與兩支 trigger 就位、`fn_warranty_expiry('2025-04-30',1,'month')`＝2025-05-31、保固類期次 0 列、13 案皆未登錄保固期間（11 案兩項皆缺、2 案只缺保固期間）。
- 合併後補正：真後端鏈號與 P3f 的 chain 14 相撞，本單元的 spec 改名為 `e2e-real/chain15-warranty.spec.js`（鏈 15），並補寫 [真後端 E2E 指南](REAL_BACKEND_E2E.md) 的該節；改名後重跑通過。
- 正式庫唯讀盤點（2026-09-20，只取計數）：保固類義務 0 筆、保固類期次 0、正式驗收合格 2 案、approved 且 `lifecycle_phase=保固` 0 筆、`trigger_config` 只有 offset／fixed 鍵。未套正式（見續接清單 §7；rebase 到含 P6b-3／P3f 的 main `1ee8f96` 後，migration 時間戳改為 `20260920040000`，`test:db` 重跑見下）。

### 2026-09-20 P3f：捨棄現場文書草稿（`codex/slimming-p3f-discard-draft`，PR #155，migration `20260920021000_field_document_discard`；rebase 到含 P6b-1／P6b-2／P6b-3 的 main `b061d13`；Opus 5 暫代 Fable 5.1）

- `npm run test:db`（一次性 DB，從零套 81 支）：59 檔、3,406 項通過、0 失敗。新增 `field_document_discard.sql` 78 條（結構、grants 與 H3 允許清單；未登入／他方／機關／非成員／跨案 `PD006`；原因空白或 null `PD010`；同方第二人捨棄 AI 起稿的文件——原因去頭尾空白、捨棄者與請求編號、時間由伺服器蓋；版本與附件保留；指向本文件的起稿與建議草稿標 `rejected` 並留稽核、指向別份的不動；`field_document.discarded` 一筆且 metadata 帶原因；冪等（同請求編號、同人同原因無編號）與 `PD009`／`PD008` 衝突、冪等與被拒都不增加稽核；捨棄紀錄 service 也不能改寫、建立時不能帶、service 無原因被 trigger 拒；已簽署／已提送／簽後更正草稿 `PD008`（guard 同樣擋）；P3e 共用補值清單與寫入都不再以捨棄的文件為對象；同日同批次重新起稿、同日第二份活文件仍 23505、新草稿補齊後可簽署並落 `daily_logs`；監造日誌與監造查驗表單捨棄後可重建且查驗申請仍待查驗；自檢表同批次同目標可再起稿、`in_review` 可捨；版本 0 的空白草稿可捨）。`field_documents.sql` 215→216（無原因捨棄被拒；原「未簽署可捨棄」改帶原因）、`field_document_sign.sql` 139（直接捨棄改帶原因）、`anon_and_function_privileges.sql` 允許清單 82→83。第一次跑 76/78：測試的「service」段沿用了前一段的 JWT（其實是廠商身分，產品正確拒絕），補 `become(null)` 後全綠；rebase 後一次跑出 `confirmed_quantity_concurrency.sql` 1 項紅（`dblink` 鎖等待計時，與本單元無關；當時同機在跑真後端與 edge runtime），單獨重跑全綠。
- `npm test`：142 檔、1,534 項通過（rebase 到含 P6b-3 後）。新增 `DiscardDraftButton.test.jsx` 6（顯示條件、原因必填對話框、取消不送、伺服器拒絕如實顯示、文件頁捨棄後回 `/site` 帶說明、簽後更正／他方／已簽署無入口）、`fieldDocs.test.js` +4（入口條件與終態）、`db.test.js` +1（只替未簽署狀態的文件查簽署列、分批）、Edge `fieldDocDraftRun.test.ts` +1（捨棄的同日文件不動、重新上傳另起一份）。
- `npm run lint` 零警告；`npm run build`；`npm run check:edge` 15 支；`npm run check:docs` 55 檔 0 錯。
- Demo E2E（自起 server、不設 `CI`）：全套 78 項通過。新增：施工日誌文件頁捨棄（取消什麼都不發生、原因必填）→ 回現場紀錄顯示原因與「施工日誌頁」連結 → 同日重新填寫存成新版本 1；自主檢查表在 `/site` 清單列捨棄，375 寬按鈕高 ≥44、無水平溢位（第一次量到 152px 是改視窗大小後立即量、版面尚未重排，改 `expect.poll` 後為 0）。rebase 到 P6b-2 後 CI 紅一輪：demo 種子自 P6b-2 起帶兩份 Agent 起稿文件（日期為今天減三天），`/site` 清單同時有兩份同類文書，新斷言改以當日日期指名（`捨棄草稿：<今天> …`、`… <今天>` 連結），產品未改。
- 真後端 E2E（本機共用棧 `supabase_db_PMIS` 以 psql 套本支並登記版本、PostgREST 重載 schema；`functions serve --env-file e2e-real/stub.env`，`deno.lock` 暫移後還原；埠 5189）：新 `e2e-real/chain14-discard-draft.spec.js` 通過（rebase 前 5.5s、rebase 後重跑 7.3s）；rebase 前 chain 5／6／8／10／12 回歸 5 項 32.8s 通過。
- 正式環境：2026-09-20 使用者親自 `supabase db push` 依序套 P4e `20260920001500`、P3e `20260920004000`、P3f `20260920021000`；`migration list --linked` 核對遠端已含三支、只剩 P6b-3 `20260920030000` 待套。合併後 `check:prod` 與正式庫唯讀核對見 [CURRENT §6.3](../CURRENT.md#63-正式環境最後核對不是即時狀態)。
- 未做：iPhone 實機；§8.1 A12 使用者驗收。

### 2026-09-20 P6b-3：品質查驗快速判定與檢查表直接登錄退場（`codex/slimming-p6b3-quality-doc-only`，PR #158，migration `20260920030000_quality_direct_writes_retire`；基準 main `cf06e2c`；Opus 5 暫代 Fable 5.1）

- `npm run test:db`（一次性資料庫從零套 migration＋seed；本機 CLI 2.113.0）：58 檔、3,325 通過、0 失敗。新增 `quality_direct_writes_retired.sql` 50 條（grant／policy 形狀；三角色＋管理者＋非成員直接判定／寫檢查紀錄皆 42501；查驗申請照常且判定欄清空、直接建成已判定 P0001、刪待查驗照常；guard 縱深防禦：非簽署路徑改 status／判定說明／判定時間／撤銷判定皆拒、非判定欄照改、已判定申報資料不可改、override 翻不過；檢查紀錄只放行同案自檢表簽署交易（他案文件、非自檢表文件、非 UUID 的 GUC 皆拒），簽署路徑判定重算與建立者照舊；查驗表單簽署路徑可判定並同交易開缺失；service 修復路徑照舊；歷史列無損）。既有 7 支改寫：`checklist_revisions`（以同案自檢表 GUC 模擬簽署路徑、跨案改用他案文件）、`inspection_checklist_link`／`evidence_guards`（歷史列改 service 建）、`self_check_documents`（§6 舊路徑改斷言 42501＋簽署交易內重算與開缺失；§10 guard 規則改以 superuser＋claims 驗）、`inspection_form_documents`（快速判定改斷言 42501／guard 訊息、缺失與稽核改走簽署路徑模擬；152 條）、`p0_05_audit_events`（判定稽核改走簽署路徑模擬）、`formal_mode`（監造直接判定被擋）。
- rollback：本機共用棧（`supabase_db_PMIS`）以 psql 套本支並登記版本 → 套 `.down.sql`（authenticated 權限、8 條 policy 回復、`fn_checklist_sign_bypass` 移除）→ 重套本支（權限收回、4 條 policy）。`.down.sql` 的兩支 guard 與 P3b／P3c 原文逐字比對相同。
- `npm test`：141 檔、1,522 項（刪 `ChecklistSection.unsaved.test.jsx`；`quality.test.js` 改為 slice 不再暴露直接寫入函式＋`ensureChecklistTemplate` 兩條；`Quality.journey.test.jsx` 改為檢查表只剩查閱、新增導向文件頁、監造詳情沒有快速判定）。`npm run lint` 零警告；`npm run build`；`npm run check:docs` 0 錯。
- Demo E2E（`E2E_DEMO_PORT=5388`）：supervisor／workflow-ux／a11y／contractor 53 項通過（監造詳情沒有快速判定、入口直達查驗表單、舊判定標章；手機抽屜只有表單入口並可返回原清單；appPrompt Esc 改用缺失「提送複查」）。
- 真後端 E2E（本機共用棧套本支；`functions serve --env-file e2e-real/stub.env`，暫移 `deno.lock` 後還原）：chain 4（品質證據改 service 建）、8（自檢表簽署落 `checklist_records`）、10（查驗表單簽署判定＋確認量）、13（月報／佐證包）4 項通過。
- 未做：正式 `supabase db push`（待使用者）；Demo 站未重佈。

### 2026-09-20 P6b-2：Agent 日誌／自主檢查表草稿改為產生現場文書草稿（`codex/slimming-p6b2-agent-field-docs`，PR #157；無 migration；基準 main `8001f3a`；Opus 5 暫代 Fable 5.1）

- `npm test`：142 檔、1,532 項通過。照片起稿抽出共用 `writeDraftDocument` 後原 `fieldDocDraftRun.test.ts` 28 項照綠，新增 Agent 起稿 5 項（日誌建立無批次文件＋AI 版本＋`draft_daily_log` 指向文件、非工地不納入；與照片起稿同一份活文件——加版本／內容相同不加／已有人工版本只留建議／已簽署不動；沒有可用照片只回說明；自檢表 agent 鍵＋勾選建議 `filled`／`ai:agent`＋實測值不帶值；未指定工項不掛照片）；`agentTools.test.ts` 改測 `validateBoolSuggestions`；白名單掃描加「草稿工具只經 `writeDraftDocument` 寫文件草稿、AI 版本、agent_actions」；`agent.test.js` 改測接受順序紅線（日誌先存數量才標已接受、存失敗維持 pending、自檢只標已接受、舊格式拒絕）；新 `fieldDocs.agentDraft.test.js` 3 項；`fieldDocs.test.js` 以 `applyInboxQuantities` 取代 `contentFromAgentDraft`。
- `npm run lint` 零警告；`npm run build`；`npm run check:edge` 15 支；`npm run test:edge` 4；`npm run check:docs` 0 錯。
- Demo E2E（`E2E_DEMO_PORT=5388`、不設 `CI`）：全 11 支 77 項通過。新增 contractor「Agent 收件匣」：日誌草稿填數量接受 → 提示版本 2 → 施工日誌頁看得到 12；自檢草稿卡片列 AI 建議、接受 → `/self-check?doc=`。demo 種子兩份 Agent 起稿文件放在 3 天前（demo 既有日誌的空缺日），今天與新自檢表的故事線不受影響；自檢表清單件數斷言 1→2。
- 本機共用棧（`supabase_db_PMIS`）一次性真 DB 驗證（Deno 腳本直接呼叫 `draftDailyLog`／`draftInspection`，一次性帳號與專案、跑完 `delete_project`＋刪帳號；腳本不進 repo）：12/12 通過——日誌文件無批次、版本 1、`created_by`＝使用者、AI 版本數量 null、日期來源 `agent:request`；`agent_actions` 指向文件且本人 RLS 可讀；重擬不加版本；使用者 `save_field_document_version` 疊數量成版本 2；已有人工版本後重擬只留 `suggest_field_update`；自檢文件 agent 鍵＋範本、勾選建議 `ai:agent`、實測值 null、附當日照片；建議指到實測值項被拒；`daily_logs`／`checklist_records` 零寫入。
- 未跑：`test:db`（無 migration）；`agent-run` 端到端（需模型金鑰，chain 3 仍擋）。

### 2026-09-20 P6b-1：移除退場頁 `/schedule`、`/audit` 與三支退場 Edge 原始碼（`codex/slimming-p6b-retire-cleanup`，PR #154；無 migration；基準 main `f257db0`；Opus 5 暫代 Fable 5.1）

- `npm test`：141 檔、1,540 項通過（P6a 基準 143 檔 1,551：刪 `Schedule.readonly.test.jsx` 3 項與 `riskAudit.test.js`；`navConfig.test.js` 改為退場路由 `access: 'retired'`——登記內容、roles 搬家不掉、承接頁對原角色可進、不在導覽定義；`aiFeatures.test.js` 目錄測試改為啟用功能必須有目錄、退場鍵原始碼必須不存在）。
- `npm run lint` 零警告；`npm run build`；`npm run check:edge` 15 支（18−3）；`npm run test:edge` 4；`npm run check:docs` 55 檔 0 錯（瘦身報告對已刪 `Schedule.jsx` 的連結改釘盤點基準 `ab4be5f`）。
- Demo E2E（`E2E_DEMO_PORT=5388`、不設 `CI`）：routes／reachability／owner／contractor／a11y 5 支 48 項通過。新斷言：廠商 `/schedule?item=<key>` 落在履約時程該項關鍵工項詳情（計畫完成日為剛改的 2099-12-31）、`/schedule` 無參數落在契約重點；機關 `/audit` 落在估驗計價「缺件與檢核」；廠商 `/audit`、機關 `/schedule` 維持無權限畫面且網址不被導走；a11y 全路由掃描不再含兩條退場路由。
- 未跑：`test:db`／真後端 E2E（無 DB、RPC 或資料流程變更）。
...
- `npm run test:db`（一次性資料庫從零套 79 支 migration＋seed，rebase 到含 P4e 後重跑）：57 檔、3,271 通過、0 失敗（rebase 前 78 支 3,220；P4e +47；P4e 交接 +4：`inspection_form_documents.sql` 146→150，v4／v5 改斷言確切訊息、PD008 加 `throws_like`、缺失說明「申報 100 M3、確認 60 M3、差額 40 M3」且無 `.0000`、直呼簽署分支驗申報不符／部分合格／不合格三處訊息）。新增 `intake_shared_inputs.sql` 95 條：結構與執行權限（兩支 RPC 開給 authenticated、helper 與套用函式 API 角色皆不可執行）、共用鍵目錄（鍵解析含不存在日期／大寫 uuid／目錄外鍵、施工日誌天氣＋工項列位置與數量、自檢表位置、查驗表單不共用、監造日誌只共用天氣）、值正規化、效果判定；清單（上傳方可補、同方文件、效果由伺服器判定、工項標籤、監造可讀不可補、非成員 `PD006`）；權限與輸入（非成員／監造／機關 `PD006`、anon `42501`、目錄外鍵／他案工項／負數量／空白位置／本批沒用到的鍵 `PD010`，被拒不留紀錄不產生版本）；補一次（三份草稿各一個人工版本、建立者＝補值者、雜湊＝DB 以內容＋附件重算、回傳雜湊＝版本雜湊、來源 confirmed／`shared:<鍵>`、附件原樣、版本說明、狀態重算、監造日誌／他案（偽造候選）／他批文件不變、批次記下補值與補值者）；冪等（同值 `updated=0`、版本數與時間不變、清單三份 applied）；簽署一份與另一份人工改寫後再補另一值（施工日誌更新、已簽署 locked 且狀態／版本／雜湊／內容不變、人親自確認 human_value 不覆蓋）、簽後更正中 locked；補齊後施工日誌 recheck 只剩監造照片附件問題、簽署 `PD005`；反向監造補自己批次只動監造日誌、廠商補監造批次 `PD006`；捨棄批次 `PD008`；套用函式確認者與時間取自補值紀錄、不合法值不套、人標不適用列 skipped。H3 允許清單 80→82（`anon_and_function_privileges.sql` 408 條）。前幾次紅燈與處理：Supabase CLI 套 migration 的敘述切割在 plpgsql 內 `… case … end then` 處誤切（`syntax error at end of input`；改成先把上限算進變數，函式語意不變）；測試寫法三處——以 authenticated 呼叫雜湊函式被權限擋（改 owner 重算）、`->` 與 `-` 運算子優先序、計數 helper 改 security definer（他案列不能以「RLS 看不到」混過）。
