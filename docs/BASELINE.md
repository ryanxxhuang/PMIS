...
- 正式庫唯讀盤點（2026-09-20，只取計數）：保固類義務 0 筆、保固類期次 0、正式驗收合格 2 案、approved 且 `lifecycle_phase=保固` 0 筆、`trigger_config` 只有 offset／fixed 鍵。未套正式（見續接清單 §7；rebase 到含 P6b-3 的 main `b061d13` 後，migration 時間戳改為 `20260920040000`，`test:db` 重跑見下）。
