...
- Demo E2E（自起 server、不設 `CI`）：contractor／supervisor／a11y／routes 4 支 49 項通過。新增：施工日誌文件頁捨棄（取消什麼都不發生、原因必填）→ 回現場紀錄顯示原因與「施工日誌頁」連結 → 同日重新填寫存成新版本 1；自主檢查表在 `/site` 清單列捨棄，375 寬按鈕高 ≥44、無水平溢位（第一次量到 152px 是改視窗大小後立即量、版面尚未重排，改 `expect.poll` 後為 0）。
