...
> ACTIVE｜2026-09-20｜P3f 捨棄現場文書草稿（pgTAP＋Vitest＋Demo E2E＋真後端 chain 14 與 chain 5／6／8／10／12 回歸）、P6b-1 移除退場頁與退場 Edge 原始碼（Vitest＋Demo E2E 5 支）、P6a 月報／監造月報／佐證包只重用已簽署資料（Vitest＋Demo E2E＋真後端 chain 13＋內建 Preview 375／1024）、P3e 共用補值（pgTAP＋Vitest＋真後端 chain 12 與 chain 5／6／8／10 回歸）、P4e 收回估驗明細直接寫入（pgTAP 全套＋rollback 實跑＋真後端 chain 2／7／9／11）、O1 demo 站重佈與 P3c 合併結果同步（`check:prod` 五頁＋正式庫唯讀核對）、T2 Demo E2E 受測 dev server 隔離（修正前後對照＋Demo E2E 全套）、T1 真後端 E2E 受測 dev server 不監看檔案（chain 5→6→7 連跑 3 輪）、P4c 估驗頁接上後端強制（Vitest＋Demo E2E＋真後端 chain 2／7／9＋內建 Preview 375／1024）、P3d 施工日誌列印與提送回執／退回歷史呈現（Vitest＋Demo E2E＋真後端 chain 5／6／8＋內建 Preview 375／1024）、P3b 自主檢查表、P4b 監造確認量後端強制（pgTAP＋dblink 真併發＋Vitest＋真後端 chain 2／7）、P6c `audit.summary` 退場（pgTAP＋Vitest＋Deno＋Demo E2E）、CI1 釘住 pgTAP workflow 的 Supabase CLI 版本（本機 pgTAP＋PR CI 實跑）、P5d 履約時程 UI（Vitest＋Demo E2E＋內建 Preview 1024／375）、R1 全面移除兩步驟驗證（pgTAP＋Vitest＋Demo E2E＋真後端 chain 1／5／6）、P5c 基準日版本（pgTAP＋Vitest 前端／Edge 路徑＋Deno 執行期＋Demo E2E＋內建 Preview）、D1 正式站邊緣注入與 `check:prod` 漏檢（Vitest＋`wrangler dev`＋demo 重佈實測）、P3a 監造日誌頁面（Vitest＋Demo E2E＋真後端 chain 6）、H2／H3 anon 與函式 EXECUTE 權限硬化（pgTAP 全庫迴圈＋真後端 E2E 四鏈）、P5b 循環義務逐期追蹤（pgTAP＋Vitest 前端／Edge 路徑＋Deno 執行期＋Demo E2E）、P3a 監造日誌後端（pgTAP＋Vitest＋Deno）、P5a 球權單一實作與共用案例（Vitest 前端／Edge 路徑＋Deno 執行期＋pgTAP）、H1 表級權限硬化（pgTAP 全表迴圈＋真後端 E2E）、P2b Edge 起稿（Vitest＋pgTAP＋Deno）、瘦身 P1b 退場頁唯讀化（成本寫入 DB 收回 pgTAP）、P2d 施工日誌存版／簽署／提送 RPC pgTAP、瘦身 P1c／P1d 文案對齊與手機抽屜斷點缺陷、P2a 現場文書資料層 pgTAP、T0 本機 pgTAP 隔離、P4a 純計算層 pgTAP；保留 2026-09-14 三方 UIUX 與 2026-09-12 的既有後端與全案驗證快照。
> 手動實跑快照，不是 CI 自動產物。前一版驗證紀錄可從 Git 追溯；正式環境狀態只見 [CURRENT §6.3](../CURRENT.md#63-正式環境最後核對不是即時狀態)。

## 1. 本輪驗證

### 2026-09-20 P3f：捨棄現場文書草稿（`codex/slimming-p3f-discard-draft`，PR #155，migration `20260920021000_field_document_discard`；rebase 到含 P6b-1 的 main `8001f3a`；Opus 5 暫代 Fable 5.1）

- `npm run test:db`（一次性 DB，從零套 80 支）：58 檔、3,352 項通過、0 失敗。新增 `field_document_discard.sql` 78 條（結構與允許清單；未登入／他方／機關／非成員／跨案 `PD006`；原因空白或 null `PD010`；同方第二人捨棄 AI 起稿文件、捨棄紀錄由伺服器寫、版本與附件保留、指向本文件的起稿與建議草稿 `rejected` 並留稽核、`field_document.discarded` metadata 帶原因；冪等與 `PD009`／`PD008` 衝突、冪等不增加稽核；捨棄紀錄 service 也不能改、建立時不能帶、service 無原因被拒；已簽署／已提送／簽後更正草稿 `PD008`；P3e 共用補值不再以捨棄的文件為對象；同日同批次重新起稿並簽署落 `daily_logs`；監造日誌與查驗表單捨棄後重建、查驗申請仍待查驗；自檢表同批次同目標再起稿、`in_review` 可捨；空白版本 0 可捨）；`field_documents.sql` 215→216（無原因捨棄被拒；原「未簽署可捨棄」改帶原因）、`field_document_sign.sql` 139（直接捨棄改帶原因）、`anon_and_function_privileges.sql` 允許清單 +`discard_field_document`。第一次跑 76/78：測試裡「service」段沿用前一段的 JWT（寫成 service 其實是廠商身分，產品正確拒絕），補 `become(null)` 後全綠。
- `npm test`：142 檔、1,552 項通過（rebase 到含 P6b-1 後）。新增 `DiscardDraftButton.test.jsx` 6（顯示條件、原因必填對話框、取消不送、伺服器拒絕如實顯示、文件頁捨棄後回 `/site` 帶說明、簽後更正／他方／已簽署無入口）、`fieldDocs.test.js` +4、`db.test.js` +1（只替未簽署狀態文件查簽署列）、Edge `fieldDocDraftRun.test.ts` +1（捨棄的同日文件不動、重新上傳另起一份）。
- `npm run lint` 零警告；`npm run build`；`npm run check:edge`；`npm run check:docs` 0 錯。
- Demo E2E（自起 server、不設 `CI`）：contractor／supervisor／a11y／routes 4 支 49 項通過。新增：施工日誌文件頁捨棄（取消什麼都不發生、原因必填）→ 回現場紀錄顯示原因與「施工日誌頁」連結 → 同日重新填寫存成新版本 1；自主檢查表在 `/site` 清單列捨棄，375 寬按鈕高 ≥44、無水平溢位（第一次量到 152px 是縮放視窗後立刻量、版面尚未重排，改 `expect.poll` 後為 0；溢位元素是可橫捲的分頁列）。
- 真後端 E2E（本機共用棧 `supabase_db_PMIS` 以 psql 套本支並登記版本、PostgREST 重載 schema；`functions serve --env-file e2e-real/stub.env`，`deno.lock` 暫移後還原；埠 5189）：新 `e2e-real/chain14-discard-draft.spec.js` 5.5s 通過；chain 5／6／8／10／12 回歸 5 項 32.8s 通過。
- 未做：正式 migration（正式庫停在 `20260919222000`，P4e／P3e／P3f 三支待使用者 `supabase db push`；PR #155 依部署順序鐵則開著不合併）；iPhone 實機；§8.1 A12 使用者驗收。
