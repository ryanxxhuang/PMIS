...
- 真後端 E2E（共用開發 DB 以 psql 套本支並登記版本，rebase 後版本號同步改為 `20260920040000`；無模型金鑰的固定資料模式；rebase 後重跑兩鏈皆通過）：新增 chain 14——廠商（管理者）補登兩條保固契約重點、監造確認 → 期程卡「缺正式驗收合格日、缺契約保固期間」、保固巡檢無期次；管理者在期程卡登錄 2 年並引用條文 → 留版、仍不產生；機關登錄正式驗收合格 2025-03-15 → RPC 期滿 2027-03-15、期次起於 2025-04 且全部 ≤ 期滿日；監造看到同一份依據、無登錄按鈕、期次依據句寫出合格日與期滿日。chain 3 通過（「擷取審核」選擇器限定主內容區，側欄同名子頁造成的既有 strict mode 衝突）。
