...
| P2a | `codex/slimming-p2a-field-docs`／PR #109 | `f5bad72`（merge commit 合併後填） | `20260917201000_field_documents`（五表／`photos` 七欄／六支 guard／稽核 trigger；rollback 檔同名 `.down.sql`） | 合併後 `supabase db push`（結果寫 CURRENT §6.3） | 本機 `npm run test:db` 從零套 62 支：42 檔 1,346 通過、0 失敗（新增 `field_documents.sql` 215/215）；lint 綠；`npm test` 120 檔 1,259 項；build 綠；`check:docs` 55 檔 361 連結 0 錯；CI 見 PR | P2b Edge 起稿（AI 重跑＝新增版本；`ai_*` 只 service 可寫）、P2c 現場紀錄頁（直接 INSERT `photo_intakes`／`photos`／`field_documents` 草稿，其餘走 RPC）、P2d RPC 依設計 §5 的寫入順序（簽署列→狀態→事實表） |
