...
CLAUDE.md §2 第一條紅線：「核定／判定／結案／驗收／凍結——agent 的工具箱裡根本沒有這些工具。靠工具白名單保證，不是靠 prompt 約束。」`agentPersona.ts` 的 `COMMON_RULES` 也叫模型不要宣稱已核定，但那是第二層；第一層是**模型呼叫得到的函式集合裡沒有任何一支能寫業務資料表**。
