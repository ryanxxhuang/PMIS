...
| `PD007` | 此文件類型的簽署尚未支援（`inspection_form`；`daily_log` P2d、`supervisor_log` P3a、`self_check` P3b 已支援） | 顯示訊息 |
...
回 `{document_id, version_no, content_hash, signature_id, signed_at, signer_id, status:'signed', target_table:'daily_logs'|'supervisor_logs'|'checklist_records', target_id, agent_actions_resolved, idempotent}`。稽核 `field_document.signed` 由簽署列的 AFTER trigger 寫，RPC 不另記。
...
| `sign_field_document` 的 `inspection_form` 分支、`create_field_document_draft`（service；P2b 起稿改由 Edge service 直接 INSERT，本 RPC 未建）、`set_intake_shared_input`、`discard_field_document` | security definer RPC | P3c／P3e |
