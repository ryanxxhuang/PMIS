...
6. 候選文書（[`inferCandidates`](../../supabase/functions/_shared/fieldDocDraft.ts)，確定性規則，不由模型決定；存進 `photo_intakes.candidates`，使用者先前的 `excluded` 沿用）：
   - `contractor`：每個日期一份 `daily_log`（`target_key`＝日期；`state=ready`）；無法判日的照片→一份 `daily_log` `state=blocked`（`blocked_by:['log_date']`，補批次日期後重試）；**每個「配到的工項 × 日期」一份 `self_check`**（P3b；`target_key`＝`<日期>:<work_item_id>`，範本由本案 `checklist_templates` 依工項描述以 [`pickChecklistTemplate`](../../supabase/functions/_shared/fieldDocDraft.ts)（與 Agent `draft_inspection` 同一支；P6b-2 自 `draftInspection.ts` 移入，免得純規則模組反向 import 寫 DB 的工具模組）確定性挑選並在 `reason` 寫理由，候選帶 `work_item_id`／`template_id`；本案沒有範本或挑不出→`state=blocked`（`blocked_by:['checklist_template']`，由人在頁面指定範本手動建立，不猜）；範本讀取失敗→不推自檢表並在 notes 揭露，不能寫成「沒有範本」；`applies_to` 規則待 P3c）。
...
- **P6b-2 落地（2026-09-20）**：兩支 Agent 草稿工具改為與照片起稿同一條路——內容由同一支 builder（`fieldDocDraft.buildDailyLogDraft`／`buildSelfCheckDraft`，讀同一份照片辨識結果）湊，寫入由從照片起稿抽出的同一段 [`writeDraftDocument`](../../supabase/functions/_shared/fieldDocDraftRun.ts)（活文件不存在→建立＋AI 版本；內容相同不加版本；已有人工版本只留 `suggest_field_update`；已簽署／提送不動），`agent_actions` 一律 `target_table='field_documents'`、`target_id`＝文件（`kind` 仍是 `draft_daily_log`／`draft_inspection`，簽署時 `resolve_agent_action_internal` 自動結案）。施工日誌：該日一份，與照片起稿同一份活文件（`intake_id`／`target_key` 皆 null）；照片取拍攝時間落在該日者（`photos.taken_at`，非工地／不可辨／重複不納入）。自主檢查表：Agent 一份一鍵（`<日期>:<工項|->:agent:<範本>`，沿用照片起稿鍵的前兩段、清單頁照舊以第二段對回工項；`intake_id` null，與批次文件不互認）；勾選建議須附依據（`validateBoolSuggestions`，指到實測值項直接拒絕），帶入為 `filled`／`ai:agent`，仍要人逐項確認才能簽；實測值永遠不帶值（DB 版本 guard 另擋）；可不指定工項（不掛照片、工項待補）。收件匣：日誌草稿的「接受」只把卡片上人填的數量疊到文件目前版本存成人工版本（`lib/fieldDocs.applyInboxQuantities`；沒填不加版本），前端 `contentFromAgentDraft` 第二份轉換刪除；自主檢查表草稿的「接受」不動內容、只標已接受並帶去 `/self-check?doc=`（收件匣一鍵不能代替逐項確認）；舊格式（target 不是文件）草稿明確拒絕（正式庫 2026-09-20 此類 0 筆）。`draft_inspection` 的接受不再呼叫 `createChecklistRecord`（直接寫 `checklist_records` 的路徑，P6b-3 收回）。
...
| `create_field_document_draft`（service；P2b 起稿改由 Edge service 直接 INSERT，本 RPC 未建）、`discard_field_document` | security definer RPC | `discard_field_document` 未建：原列 P3e，P3e 範圍只含共用補值，拆為續接清單 **P3f**（authenticated 對 `field_documents` 沒有 UPDATE grant，目前沒有任何捨棄文件草稿的路徑；guard 已限「只限未簽署」） |
...
- 既有 `checklist_records`（5）不包裝成文件、也不重算判定（P3b 的伺服器重算只在新的使用者路徑 INSERT 生效）。**P6b-3（migration `20260920030000_quality_direct_writes_retire`）起直接登錄路徑退場**：品質頁檢查表分段只剩查閱（列紀錄、檢附查驗、列印；「新增自主檢查表」導向 `/self-check`），`authenticated` 對 `checklist_records` 無 INSERT／UPDATE／DELETE、寫入 policy 刪除，`checklist_records_guard` 另要求使用者路徑的 INSERT 在同案自主檢查表的簽署交易內（`fn_checklist_sign_bypass` 讀 `pmis.field_document_sign`）；舊流程直接登錄的 5 筆照常可讀、可列印、可檢附，但不再能就地修訂（更正＝新的自主檢查表文件簽署）。同一支 migration 收回 `inspections` 的 UPDATE：品質頁「合格／不合格」快速判定退場，判定（`status`／`result_note`／`inspected_by`／`inspected_at`）只由監造查驗表單簽署寫入（guard 對非簽署路徑一律拒，含 admin override；查驗申請建立時判定欄一律清空）；正式 10 筆舊快速判定照常保留，詳情標「舊流程快速判定，未填確認數量」，要更正＝由該查驗建立監造查驗表單並簽署（簽署路徑可覆寫尚無表單判定的查驗）。收緊型：先前端後 DB，service role 修復路徑不變；回復檔 `supabase/rollbacks/20260920030000_quality_direct_writes_retire.down.sql`（本機共用棧實跑回復再重套）。既有 `daily_logs`（12）、`checklist_records`（5）、`inspections`（11）不自動包裝成已簽署文件；使用者開啟時可「建立文件草稿（沿用既有內容）」再走簽署，`field_sources` 全標 `filled/source: legacy`。**P2c 落地**：`/site-log` 開到「有 `daily_logs` 列、沒有活文件」的日期時，以該列內容帶入表單（[`contentFromLegacyLog`](../../src/lib/fieldDocs.js)：有值的欄 `filled/legacy:<id>` 並標「既有紀錄、待核對」，空欄 `pending`；工項鍵由 `item_key` 換成 uuid，換不到的保留原鍵由簽署 `PD010` 擋下），第一次存檔才 INSERT 文件並存版本；狀態章寫「既有紀錄・未簽署、待核對」，列印仍只印已落庫的列。前端不再有直接寫 `daily_logs` 的路徑（`saveSiteLog`／`deleteSiteLog` 已移除；未簽署舊列要更正只能走文件→簽署覆寫）。
...
- 相容順序：DB（加法）→ Edge（新函式；舊 `draft_daily_log` 工具仍可用）→ 前端 → 第二支 migration 才把 `acceptDraft` 舊路徑用到的直接寫入關閉。（P6b-2 起 `acceptDraft` 已無任何直接寫事實表的路徑。）
