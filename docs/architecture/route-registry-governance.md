...
今日待辦連結的 router state 帶返回來源（`taskReturn.js` 單一定義：`/dashboard`、`/alerts`、`/site`）；`useListDetailPane` 更新 query 時保留 state。Dashboard 的搜尋、類型、到期篩選與載入筆數在 query；回來可還原原篩選並聚焦原筆（已完成則移到搜尋）。
