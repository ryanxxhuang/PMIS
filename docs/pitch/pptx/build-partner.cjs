...
//   2) 時程不寫死；價格只寫死在 P24 那一頁（使用者 2026-08-20 指定的級距），
//      其他頁一律不要再出現金額，否則兩處會漂。
//
...
//
// 2026-08-20 追加:客群改為建築師事務所與顧問公司(不再直接找機關),
// 資安整頁(原 P15 合規表)也移進 --full。銷售版只在紅線頁用一段話帶過:
// 「我們做公共工程,還是會符合政府的資安要求」。細節等對方問了再翻完整版。
...
// ═══════════════════════════════════════════════════════════════════════════
{
  ...
  cite(s, M, y + 4.3, CW, 0.72, '另外兩件',
    '每個 AI 功能都是可獨立開關的模組——不接受把資料送到境外模型，整組關掉，其他功能照常運作。'
    + '資安則依公共工程的政府採購要求建置：弱點掃描無高風險、公開的漏洞回報機制、日誌含 IP 位址保存六個月、權限一律在伺服器端檢查。')
}
...
// ═══════════════════════════════════════════════════════════════════════════
{
  ...
  const steps = [
    ...
    ['④', '年度訂閱', '正式合作', '貴所一份年度訂閱、案件數不限，三方（廠商／監造／機關）都進來。此時才需要走採購程序，屆時再談甲乙丙哪種掛名方式。'],
  ]
  const COST = ['30 分鐘，現在就可以', '貴所出一份文件，我方數小時', '綁一個案子，三到六個月', '依規模級距計，走採購程序']
  ...
  cite(s, M, y + 3.74, CW, 0.86, '為什麼建議從②開始',
    '示範資料再漂亮，都回答不了貴所真正的問題：「我的契約丟進去，抽得出東西嗎？」跑一次真文件只花我方幾個小時，卻是唯一能讓雙方同時知道值不值得繼續的做法。貴所不必先付費、不必先簽約，只要願意提供一份文件。走到第④格才會談到錢，價目在下一頁。')
}

// ═══════════════════════════════════════════════════════════════════════════
// 24 價格
// ═══════════════════════════════════════════════════════════════════════════
// ⚠️ 這一頁是本檔唯一寫死價格的地方（使用者 2026-08-20 指定），改價只改這裡。
//    級距金額由使用者給定；AI 額度與超額單價是從正式站 ai_usage_events 的
//    實測單次成本回推的，改版前請重跑那份查詢，不要沿用下面的數字。
//    對外只賣「次數」，對內仍按 token 計量——兩者在 /admin 對得起來。
{
  const s = slide(SEC.go)
  const y = head(s, '價格', '兩張帳單：系統看規模，AI 用量按次算。')

  const rows = [
    [th('年度經手工程總額'), th('平台費（年）'), th('含 AI 次數／年'), th('含文件解析／年')],
    [tdb('3 億以下'), td('NT$ 12 萬', { bold: true, color: C.steelText }), td('4 萬次'), td('30 份')],
    [tdb('3 億 – 10 億'), td('NT$ 30 萬', { bold: true, color: C.steelText }), td('10 萬次'), td('80 份')],
    [tdb('10 億 – 30 億'), td('NT$ 60 萬', { bold: true, color: C.steelText }), td('20 萬次'), td('160 份')],
    [tdb('30 億 – 100 億'), td('NT$ 120 萬', { bold: true, color: C.steelText }), td('40 萬次'), td('320 份')],
  ]
  table(s, M, y, CW, rows, [3.3, 2.5, 3.0, CW - 8.8], { size: 11, pad: 10 })

  // 兩個超額單價卡窄、政策卡寬——價格是要被記住的數字，政策是要被讀完的句子。
  const gap = 0.34, pw = 2.9, ww = CW - pw * 2 - gap * 2
  const cy = y + 2.5
  const price = (x, label, fig, unit, sub) => {
    card(s, x, cy, pw, 1.52)
    s.addText(label, {
      x: x + 0.24, y: cy + 0.18, w: pw - 0.48, h: 0.24, margin: 0, valign: 'middle',
      fontFace: F, fontSize: 9.5, bold: true, charSpacing: 1.4, color: C.ink3,
    })
    s.addText([
      { text: fig, options: { fontSize: 29, bold: true, color: C.safety } },
      { text: '　' + unit, options: { fontSize: 12.5, bold: true, color: C.ink2 } },
    ], {
      x: x + 0.24, y: cy + 0.46, w: pw - 0.48, h: 0.5, margin: 0, valign: 'middle', fontFace: F,
    })
    s.addText(sub, {
      x: x + 0.24, y: cy + 1.0, w: pw - 0.48, h: 0.36, margin: 0, valign: 'top',
      fontFace: F, fontSize: 10.5, color: C.ink2, lineSpacing: 15,
    })
  }
  price(M, '超額 · AI 次數', 'NT$ 1', '／次', '問答、照片辨識、草稿、審查意見')
  price(M + pw + gap, '超額 · 文件解析', 'NT$ 40', '／份', '契約與規範整包、履約要求抽取')

  cardText(s, M + (pw + gap) * 2, cy, ww, 1.52,
    '額度怎麼算',
    '確定性的部分不佔額度——天氣帶入、金額與期限計算、報表輸出、提醒信都不呼叫模型。額度年度制、不遞延；用到八成與用滿各通知一次，超額不斷線，按次併入次月帳單。',
    { titleSize: 14.5, bodySize: 10.5, compact: true })

  cite(s, M, cy + 1.72, CW, 0.96, '單價出處',
    '額度與超額單價由正式站 2026-08-11～08-18 實測用量回推（共 41 次呼叫，匯率 NT$32／US$）：契約整包解析 NT$10.6／份、履約要求抽取 NT$15.1／份、Agent 問答 NT$0.72／次、施工照片辨識 NT$0.17／張、白板辨識 NT$0.11／次、估驗說明草稿 NT$0.06／次。「年度經手工程總額」指前一年度監造／專管契約對應之工程預算金額合計；100 億以上另議。')
}
...
// ═══════════════════════════════════════════════════════════════════════════
{
  ...
  asks.forEach(([tag, q], i) => {
    const x = M + i * (cw + gap)
    card(s, x, y + 2.2, cw, 1.9)
    s.addText(tag, {
      x: x + 0.24, y: y + 2.42, w: cw - 0.48, h: 0.26, margin: 0, valign: 'middle',
      fontFace: F, fontSize: 9.5, bold: true, charSpacing: 1.4, color: C.ink3,
    })
    s.addText(q, {
      x: x + 0.24, y: y + 2.76, w: cw - 0.48, h: 1.1, margin: 0, valign: 'top',
      fontFace: F, fontSize: 13, bold: true, color: C.ink, lineSpacing: 20,
    })
  })

  cite(s, M, y + 4.32, CW, 0.56, '示範站',
