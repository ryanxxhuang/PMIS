...
維護兩組日期式簡報來源：[合作簡報](build-2026.cjs) 與 [青創進駐](README-青創進駐.md)。修改產生器後重建，不直接改生成 PPTX。2026-08-12／08-19 舊版產生器與其專用 deck.cjs 已退場，歷史內容用 `git show 6987ef7:<路徑>` 追溯。

簡報的數字、價格、採購說法與 Google 風格是當時的交付快照，**不是目前產品規範**；重建檔案不代表對外內容已重新核准。現行產品採 [Apple 規範](../../UIUX-Apple-設計規範.md)，D-019 已改為 AI 契約轉錄自動確認。發布新版時另行校對文案／法源／數字與視覺。
...
產品規模與實跑結果以 [BASELINE](../../BASELINE.md) 為準，不在本指南維護第二份過時測試數字。下列成本／價格是原簡報的歷史推導，更新報價需重新量測並由使用者決定。
