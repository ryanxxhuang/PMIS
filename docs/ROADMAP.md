...
- ~~循環義務目前只算下次日期；逐期資料、月末／跨年／29～31 日、基準日更正與準時率需產品決策＋migration／pgTAP。~~ P5b（逐期、月末／跨年）與 P5c（基準日版本、重算只動未完成、停止條件）已落地。保固類循環義務的停止條件 P5e 已實作（保固期滿日＝正式驗收合格日＋契約保固期間；PR #156）；逐期準時率的完整呈現 P5d 已落地。
...
- DB 顧問問題需重核：熱點／FK 索引、auth_rls_initplan、重複 permissive policy、search_path、security-definer grants、pg_net schema。API 角色的 TRUNCATE／REFERENCES／TRIGGER／MAINTAIN 已於 `20260917213900` 收回並修 default privileges；anon 表級 DML／序列與 anon／authenticated／PUBLIC 的函式 EXECUTE default 已於 `20260919003000`（H2／H3）收回並改為 authenticated 明示允許清單，見 [續接清單 §4 H](reviews/2026-09-17-product-slimming-worklog.md)；顧問其餘項目未重核。
