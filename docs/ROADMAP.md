...
Git 整併與前端自動部署已完成；以下正式後端與驗收仍需獨立執行。

- DB／Edge 部署順序與冒煙依 [部署指南](operations/deploy.md)；前端 main 建置成功不代表後端已同步。
- 正式庫先核 migration tracker；三支新 migration `20260911100000`／`20260911100100`／`20260911110000` 尚未套用。D-022 已提交，不是待實作。
- `_shared/` 改動需要 17 支 Edge 一併重佈並核版本；用有效模型金鑰重跑 live 真後端 E2E；本輪固定資料模式與 Deno 檢查已通過，見 [BASELINE](BASELINE.md)。
