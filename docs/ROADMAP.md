...
- 進度口徑待決（2026-09-16 補強包 C，詳見 `docs/reviews/2026-09-16-uiux-w07-progress-caliber.md` §5）：C1 預定進度表月份列是月底或月 1 日累計（決定後 `plannedPctNow` 或月報取值二擇一修正）、C2 監造報表進度是否隨報告月份回看、C3 未核定估驗是否計入進度（六處含 `portfolio_summary` 須同步）、C4 月報收款／請款期數是否按月截止；W07 的標示文案（該文件 §4）待 C1 確認後實作。
