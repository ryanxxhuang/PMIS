...
test('鏈 6:監造上傳→起稿→到場親自確認→簽署→提送機關→機關退回→補正再送→機關收件;廠商唯讀、到場未確認與廠商照片被伺服器擋', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: '本日天氣下午' }).fill('多雲')
  ...
  await expect(page.getByText('框架【示範範本】supervisor_log_demo v1')).toBeVisible({ timeout: 30_000 })
  ...
  // ── 監造(手機 375):看到退回原因 → 更正重要事項紀錄 → 存檔版本 5 → 重簽 → 再送(diff 由 DB 算);375 無溢位 ──
  // C2:紙本是工程會附表五,內容欄 note 印在第五節「其他約定監造事項」的「重要事項紀錄」格(原表欄名)
  ...
  await page.getByRole('textbox', { name: '重要事項紀錄' }).fill('本日無查驗申請；已電話通知廠商明日申請 4F 模板查驗')
  await page.getByRole('button', { name: '存檔', exact: true }).click()
