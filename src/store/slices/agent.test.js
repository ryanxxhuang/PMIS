// 批3／P2c:日誌草稿 payload 的純函式測試(收件匣卡片數量合併、待填計數)。
// 「數量誠實原則」:qty_today 為 null(待人填)的工項不得變成 0;接受後走現場文書
// (lib/fieldDocs.contentFromAgentDraft,該檔另有測試),沒填的在文件裡仍是 pending 待補。
...
import { draftNeedsInputCount, applyDraftQuantities, draftPayloadToChecklist, checklistDraftCounts, extractInvokeError } from './agent.js'
...
describe('applyDraftQuantities(收件匣卡片填的數量 → 合併回草稿 payload)', () => {
  it('合併:填了的工項 qty_today 更新且不再 needs_input,未填(undefined/空字串/0/非數字)的維持原樣', () => {
    const merged = applyDraftQuantities(payload, { 'uuid-a': '30', 'uuid-c': '', 'uuid-x': 'abc' })
    // uuid-a 填 30 → 更新;uuid-b 原本就有 120 → 不動;uuid-c 空字串=未填、0 也未填 → 原樣
    expect(merged.items['uuid-a']).toEqual({ qty_today: 30, needs_input: false, source: null })
    expect(merged.items['uuid-b'].qty_today).toBe(120)
    expect(merged.items['uuid-c']).toEqual(payload.items['uuid-c'])
    // 填了的不再算「待填」;沒填的照算
