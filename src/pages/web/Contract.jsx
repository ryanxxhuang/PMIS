...
export default function Contract() {
  ...
  const canUploadDocs = isPersistedProject && can.manageDocuments
  ...
  const handleFiles = useCallback(async (fileList, targetPackage) => {
    ...
    try {
      ...
      if (pkg.package_type === 'construction' && can.manageObligations && contractTexts.length) {
  ...
  }, [canUploadDocs, packageOptions, ensurePackage, pid, currentUser, can.manageObligations,
    obligations.length, parseContractFromText, reloadRuns])
  ...
  return (
    <div className="space-y-5">
      <div className="min-w-0">
        <PageHeader title="契約文件" tagline="一次上傳,自動整理" subtitle="把整包契約文件直接拖進來,系統會自動分類、整理並找出履約要求" />
      </div>
                ...
                {list.map(({ run, doc, version }) => {
                  ...
                  return (
                    <div key={run.id} className="flex items-center gap-2 text-xs border border-[var(--border)] rounded-lg px-3 py-1.5 mb-1">
                      ...
                      {can.manageDocuments && run.metadata?.requirement_extraction === 'failed' && (
      ...
      <Card title="義務時程" action={
        can.manageObligations && selectedPackage?.package_type === 'construction' && (
