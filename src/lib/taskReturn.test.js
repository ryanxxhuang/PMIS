...
describe('taskReturn(待辦返回來源的單一定義)', () => {
  it('來源清單釘死:今日工作、提醒中心、現場紀錄,且都是登記過的路由', () => {
    expect(TASK_RETURN_SOURCES).toEqual({ '/dashboard': '今日工作', '/alerts': '提醒中心', '/site': '現場紀錄' })
  ...
  it('taskReturnState:來源頁帶 pathname+search 與表上的標籤;非來源頁回 undefined', () => {
    expect(taskReturnState({ pathname: '/dashboard', search: '?q=第 4 期' }, 'k1'))
      .toEqual({ taskReturn: { to: '/dashboard?q=第 4 期', label: '今日工作', key: 'k1' } })
