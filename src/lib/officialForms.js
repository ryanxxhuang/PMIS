...
export const SELF_CHECK_MAPPING = Object.freeze([
  ...
  row('實際檢查情形(載明檢查數值及單位)', 'results.<no>', '人親自量測填寫;紙本實測欄已寫好的數值(含兩向尺寸、多個編號)由系統照原文抄錄並標待確認', ['contractor'], '逐項必填(單一讀數住在 results[no].value;兩向尺寸／多編號分列住在 results[no].readings,兩者擇一);不適用須填原因', '項目表'),
...
export const INSPECTION_FORM_MAPPING = Object.freeze([
  ...
  row('實際抽查情形(載明抽查數值及單位)', 'results.<no>', '監造親自量測填寫;紙本實測欄已寫好的數值(含兩向尺寸、多個編號)由系統照原文抄錄並標待確認', ['supervisor'], '有範本時逐項必填(單一讀數住在 results[no].value;兩向尺寸／多編號分列住在 results[no].readings,兩者擇一);本次未查須標不適用並填原因', '項目表'),
