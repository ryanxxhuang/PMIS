    ...
    <div className="space-y-3">
      {!compact && !fixedDate && (
        <p className="text-footnote text-[var(--text-2)]">
          選好照片就會保存到伺服器並開始辨識；{org === 'contractor' ? '系統會依日期自動擬好施工日誌草稿，並為每個配到工項的照片擬自主檢查表草稿（依本案檢查表範本；實測值一律由你親自量測填寫），列出待補欄位。' : '系統會依日期自動擬好監造日誌草稿（示範範本），到場人員需親自填寫確認；日期或工項相符的待查驗申請會擬監造查驗表單草稿，判定與確認數量一律由你親自填寫。'}
