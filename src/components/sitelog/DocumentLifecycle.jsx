// 文件生命週期卡(P2c;設計 §4–§6):狀態、版本與雜湊、簽署(平台帳號＋兩步驟驗證,意願文字明示)、
// 提送給監造、監造收件／退回(必填原因)、歷次退回原因與再送差異、回執與下一責任方。
...
import { docStatusMeta, formatHash, signIntentText, ORG_LABEL, changedKeysLabel, fieldLabel } from '../../lib/fieldDocs.js'
...
export default function DocumentLifecycle({
  ...
  const intent = version ? signIntentText({ docDate: doc.doc_date, versionNo: doc.current_version_no, contentHash: version.content_hash }) : ''

  const sign = async () => {
    if (!(await appConfirm({ title: `簽署 ${doc.doc_date} 施工日誌（版本 ${doc.current_version_no}）？`, body: `${intent}\n\n簽署後內容雜湊、簽署者與伺服器時間會留存；之後更正必須另開版本重新簽署。`, confirmLabel: '簽署' }))) return
    onSign?.(intent)
  }
  const submit = async () => {
    if (!(await appConfirm({ title: `提送給監造？`, body: `將提送 ${doc.doc_date} 施工日誌版本 ${doc.current_version_no}（雜湊 ${formatHash(version?.content_hash)}）給監造；重試會沿用同一筆送件，不會重複提送。`, confirmLabel: '提送' }))) return
    onSubmit?.()
  }
  const receive = async () => {
    if (!(await appConfirm({ title: '收件此版本？', body: `收件後廠商不可再修改版本 ${doc.current_version_no}；如需更正只能另立新文件。`, confirmLabel: '收件' }))) return
    onReceive?.()
  }
  const ret = async () => {
    const reason = await appPrompt({ title: `退回 ${doc.doc_date} 施工日誌`, label: '退回原因（必填，廠商會看到）', required: true })
  ...
  return (
    <section aria-label="文件狀態與簽署" className="rounded-lg border border-[var(--border)] p-4 space-y-3 text-body">
      ...
      {/* 退回:最新原因永遠在最上面(廠商第一眼要看到為什麼) */}
      {doc.status === 'returned' && lastReturn && (
        <div className="rounded-lg bg-[var(--red-tint)] p-3 text-footnote">
          <div className="font-medium text-[var(--red-text)]">監造退回（版本 {lastReturn.version_no}，{fmtTs(lastReturn.created_at)}）</div>
      ...
      {mine && ['draft', 'pending_input', 'in_review', 'returned'].includes(doc.status) && canAct && (
        <div className="space-y-2">
          {doc.status === 'returned' ? null : pendingCount > 0 || recheck.length > 0 ? (
            <p className="text-footnote text-[var(--amber-text)]">尚有 {pendingCount} 個必填欄位待補{recheck.length > pendingCount ? `、${recheck.length - pendingCount} 個附件問題` : ''}；補齊並存檔後才能簽署。</p>
          ) : dirty ? (
      ...
      {mine && canAct && doc.status === 'signed' && (
        <div className="flex items-center gap-2 flex-wrap">
          <Button onClick={submit} busy={busy === 'submit'}>提送給監造</Button>
          <span className="text-caption text-[var(--text-3)]">提送的是已簽署的版本 {doc.current_version_no}；監造收件或退回會顯示在這裡。</span>
        </div>
      )}
      {doc.status === 'submitted' && latestSubmit && (
        <p className="text-footnote text-[var(--text-2)]">已於 {fmtTs(latestSubmit.created_at)} 提送給{ORG_LABEL[latestSubmit.to_org] || latestSubmit.to_org}（版本 {latestSubmit.version_no}）；等待監造收件。回執編號 {String(latestSubmit.id).slice(0, 8)}。</p>
      )}
      {/* 監造:收件／退回(必填原因) */}
      {!mine && viewerOrg === 'supervisor' && canAct && ['submitted', 'received'].includes(doc.status) && (
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className="text-caption text-[var(--text-3)]">收件＝確認已收到此版本；退回會要求廠商補正後重新簽署再送。</span>
        </div>
      )}
      {doc.status === 'received' && submissions.find((s) => s.action === 'receive') && (
        <p className="text-footnote text-[var(--green-text)]">監造已於 {fmtTs(submissions.find((s) => s.action === 'receive').created_at)} 收件（版本 {submissions.find((s) => s.action === 'receive').version_no}）；本文件不可再修改。</p>
      )}
              ...
              <li key={s.id} className="flex items-start gap-2">
                ...
                <div className="min-w-0">
                  ...
                  {s.diff && Array.isArray(s.diff.changed_keys) && (
                    <div className="text-[var(--text-2)]">相對退回版本 {s.diff.against_version_no} 的差異：{s.diff.changed_keys.length ? changedKeysLabel(s.diff, content).join('、') : '無頂層欄位變更'}</div>
                  )}
                </div>
              </li>
            ))}
      ...
      {recheck.length > 0 && (
        <div className="text-caption text-[var(--text-3)]">伺服器待補清單：{recheck.map((r) => fieldLabel(r.key, content)).join('、')}</div>
      )}
      {!mine && viewerOrg === 'owner' && <p className="text-caption text-[var(--text-3)]">機關為查閱視角；提送對象是監造。</p>}
