...
// 每個控件都掛 aria-label:版面走 placeholder 當提示(緊湊的一列式時點編輯器沒位置放
// 標籤),但 placeholder 一打字就消失,報讀器與 a11y 掃描都不能只靠它。
    ...
    <ModalShell open={manualOpen} onClose={() => onClose()} title="手動新增契約重點" size="xl">
      ...
      <div className="space-y-2">
        <Input value={manualDraft.title} onChange={(e) => setManualDraft((d) => ({ ...d, title: e.target.value }))}
          placeholder="標題(例:開工前 14 日內提送施工計畫)" aria-label="標題" />
        <Textarea rows={2} value={manualDraft.description}
          onChange={(e) => setManualDraft((d) => ({ ...d, description: e.target.value }))}
          placeholder="補充描述(可留白)" aria-label="補充描述" />
        <div className="flex flex-wrap gap-2">
          <Select value={manualDraft.requirement_type} className="flex-1 min-w-[8rem]" aria-label="類型"
          ...
          <Select value={manualDraft.lifecycle_phase} className="flex-1 min-w-[8rem]" aria-label="階段"
            onChange={(e) => setManualDraft((d) => ({ ...d, lifecycle_phase: e.target.value }))}>
            {['開工前', '施工中', '完工', '保固'].map((p) => <option key={p} value={p}>{p}</option>)}
          </Select>
          <Select value={manualDraft.responsible_party_type} className="flex-1 min-w-[8rem]" aria-label="責任方"
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Select value={manualDraft.dueMode} className="w-auto" aria-label="時點方式"
          ...
          {manualDraft.dueMode === 'relative' && (<>
            <Select value={manualDraft.trigger_event} className="w-auto" aria-label="基準日"
              onChange={(e) => setManualDraft((d) => ({ ...d, trigger_event: e.target.value }))}>
              {MANUAL_TRIGGERS.map(([k, v]) => <option key={k} value={k}>{v}</option>)}
            </Select>
            <Select value={manualDraft.offset_dir} className="w-auto" aria-label="基準日之前或之後"
              onChange={(e) => setManualDraft((d) => ({ ...d, offset_dir: e.target.value }))}>
              <option value="after">後</option>
              <option value="before">前</option>
            </Select>
            <Input type="number" min="1" className="w-24" value={manualDraft.offset_days} aria-label="天數"
          ...
          {manualDraft.dueMode === 'fixed' && (
            <Input type="date" className="w-44" value={manualDraft.fixed_date} aria-label="指定日期"
          ...
          {manualDraft.dueMode === 'monthly' && (<>
            ...
            <Input type="number" min="1" max="31" className="w-24" value={manualDraft.monthly_day} aria-label="每月幾號"
          ...
          {manualDraft.dueMode === 'weekly' && (<>
            ...
            <Select value={manualDraft.weekly_weekday} className="w-auto" aria-label="每週星期幾"
          ...
          {manualDraft.dueMode === 'quarterly' && (<>
            ...
            <Select value={manualDraft.freq_month} className="w-auto" aria-label="每季第幾個月"
            ...
            <Input type="number" min="1" max="31" className="w-24" value={manualDraft.freq_day} aria-label="每季第幾日"
          ...
          {manualDraft.dueMode === 'yearly' && (<>
            ...
            <Input type="number" min="1" max="12" className="w-24" value={manualDraft.freq_month} aria-label="每年幾月"
              onChange={(e) => setManualDraft((d) => ({ ...d, freq_month: e.target.value }))} placeholder="幾月" />
            <span className="text-xs text-[var(--text-2)]">月</span>
            <Input type="number" min="1" max="31" className="w-24" value={manualDraft.freq_day} aria-label="每年幾日"
              onChange={(e) => setManualDraft((d) => ({ ...d, freq_day: e.target.value }))} placeholder="幾日" />
            <span className="text-xs text-[var(--text-3)]">日</span>
          </>)}
        </div>
        {packages.length > 1 && (
          <div className="flex items-center gap-2">
            ...
            <Select value={manualDraft.contract_package_id || packages.find((cp) => cp.package_type === 'construction')?.id || packages[0]?.id || ''}
              className="flex-1" aria-label="所屬契約"
        ...
        <Input value={manualDraft.acceptance_criteria} aria-label="允收標準"
          onChange={(e) => setManualDraft((d) => ({ ...d, acceptance_criteria: e.target.value }))}
          placeholder="允收標準(可留白)" />
        <div className="flex flex-wrap gap-2">
          <Input value={manualDraft.source_clause} className="flex-1 min-w-[8rem]" aria-label="出處條款"
            onChange={(e) => setManualDraft((d) => ({ ...d, source_clause: e.target.value }))}
            placeholder="出處條款(例 5.3,可留白)" />
          <Input value={manualDraft.source_page} className="flex-1 min-w-[8rem]" aria-label="出處頁碼"
