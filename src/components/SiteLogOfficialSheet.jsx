// 公共工程施工日誌 A4 文件本體（工程會 101.10.17 修正公定格式）——表頭到簽章。
// 從 SiteLogPrint 抽出共用（S-8 紙本化）:列印頁與唯讀檢視（監造/機關）吃同一份紙本版面,
...
export default function SiteLogOfficialSheet({ project, log, siteLogs, itemList, className = '' }) {
  ...
  const ex = log.extras || {}
  // 日曆天以「開工日」起算:實際開工（commencement_date,由專案文件→基準日登錄）常晚於
  // 契約起始 start_date,拿 start_date 算會虛增天數;無開工基準日才回退 start_date。
  const dayAnchor = project?.commencement_date || project?.start_date
