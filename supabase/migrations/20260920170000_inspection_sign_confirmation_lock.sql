-- E 包｜查驗表單簽署寫確認量時，補上與 issue_supervisor_certificate 同一把逐工項交易鎖。
--
-- 問題（本輪 E 包整條流程驗收時查出，f99bfee 起算的 main 上仍成立）：
--   public.field_document_sign_inspection_form_internal 在寫 inspection_confirmations 前，
--   先「讀此 (工項, 批次, 階段) 最新一筆 active 的 qty_cum」再「插入 prev_cum + 本次確認」。
--   這段 read-then-insert 沒有取 fn_cq_lock_internal(project, work_item)——而同樣寫確認量的
--   issue_supervisor_certificate（20260919140000 §12）在讀寫前就取了（該檔 lock_timeout 5s ＋ advisory lock）。
--   兩份不同的查驗表單同時對同一 (工項, 批次, 階段) 簽署時，在 READ COMMITTED 下兩邊都讀到同一個
--   prev_cum，於是：
--     * 兩筆都以 supersedes_id = null 落庫 → 本來應該是線性的累計鏈分岔。分岔之後
--       「有效量＝Σ qty_delta」與「最新一筆的 qty_cum」不再一致（例：先 60、再 40，
--       Σdelta=100 但最新 qty_cum=40；之後第三筆只會相對其中一條分支算差額）。
--     * 或者先提交的那筆讓後者的 guard 算出負的 qty_delta，使用者收到莫名其妙的
--       「減量必須填寫原因」(VQ005)——他明明是在確認一筆新的量。
--   inspection_confirmations_guard（20260919140000）本身也是「讀最新 active 再算 delta」，
--   同樣要在鎖內才有意義，所以鎖必須由呼叫端取，guard 無法自救。
--
-- 做法：把鎖補在工項與批次都已驗證完之後、任何一次讀 inspection_confirmations 之前
--   （v_batch 指派之後）。整支函式其餘部分逐字沿用 20260920004000 的版本，只多這兩行。
--   advisory 鎖是交易級且同一 session 可重入，AFTER trigger 走 fn_cq_allocate_to_cap_internal
--   再取同一把鎖不會自我死鎖；先取鎖也讓本路徑與 issue_supervisor_certificate 的取鎖順序一致。
--
-- 資料：不動任何一列，只換函式定義。
-- 回復：supabase/rollbacks/20260920170000_inspection_sign_confirmation_lock.down.sql
--   （把函式換回 20260920004000 的版本，即移除這兩行；沒有資料需要還原）。
-- 測試：supabase/tests/inspection_form_documents.sql 新增「簽署交易內確實持有該工項的 advisory 鎖」。
...
comment on function public.field_document_sign_inspection_form_internal(public.field_documents, public.field_document_versions, uuid) is
  'P3c(E 包起:寫確認量前取逐工項 advisory 鎖,與 issue_supervisor_certificate 同一把):監造查驗表單簽署分支——驗查驗／工項／單位／位置／階段／申報量／判定／確認量／查驗項目後,更新 inspections(簽署即判定)並依判定寫入 inspection_confirmations(累計;同查驗同量冪等、不同量須先撤銷);缺失由 inspections_defect_sync 開;只由 sign_field_document 呼叫。';
