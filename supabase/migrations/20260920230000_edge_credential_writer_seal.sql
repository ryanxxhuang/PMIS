-- F2｜服務憑證(Edge service role)不得寫入監造確認量與估驗狀態:三支寫入者改「只認內部旗標」。
--
-- 問題(F2 2026-09-20 以本機隔離棧實測,388daa4 的 main 仍成立;探針以 set local role service_role、auth.uid() 為 null 執行):
--   1. inspection_confirmations_guard 的 INSERT 分支驗的是「內容是否合法」(工項可計價、單位相符、確認人是本案監造成員、
--      查驗已判定、文件已簽…),**沒有驗「是誰在寫」**。服務憑證只要湊出一筆內容合法的列(confirmed_by 填任一位監造成員),
--      就能憑空落一筆 active 確認量——實測 INSERT 成功、qty_delta=60,而監造本人從頭到尾沒有簽任何東西。
--      active→revoked 的 UPDATE 分支同樣只驗「有沒有填原因」,服務憑證也能替監造撤銷。
--   2. valuations_guard 的角色與狀態機檢查全部掛在 is_user(auth.uid() is not null)之下;非登入者(服務憑證、DBA 直連)
--      不受狀態機約束:實測可直接 insert 一筆 status='已核定' 的期別,再 update 成 '已請款'。valuations_checkpoint_guard
--      只驗數量不變量(AFTER UPDATE;INSERT 不驗),所以「沒有明細的期別」可以被服務憑證直接核定與請款;有明細的期別在
--      廠商備妥、等監造核定時,也可以被服務憑證代替監造核定(數量檢查通過即可)。
--   3. work_item_pricing_basis_guard 只驗工項屬於本案;服務憑證可直接 insert／update 計價依據(例如把工項標 excluded 或改
--      pro_rata 規則),而計價依據決定可估驗上限的算法。set_work_item_pricing_basis 是唯一合法寫入者(登入的監造)。
--   Edge 目前沒有任何一支寫這三張表(_shared/valuationWrites.scan.test.ts 釘住原始碼),所以這是「守衛缺口」不是「已發生的
--   事故」;但 D-026／設計 §9 的前提是 RLS／RPC／guard 是安全邊界、前端與 Edge 原始碼掃描只是 UX 與縱深,缺口必須在 DB 補。
--
-- 做法(比照 P4e 20260920001500 對 valuation_items 的封堵:所有寫入者一體適用,只認交易內 GUC pmis.cq_internal):
--   1. inspection_confirmations_guard:INSERT 一進來就要 fn_cq_internal();active→revoked 也要。其餘內容檢查、
--      RI set-null 放行、DELETE 只放行 cascade,逐字不變。訊息代碼 VQ010(與「只能由重算路徑寫入」同碼)。
--   2. 三個合法寫入者各自只在「那一句 insert／update」前後開關旗標(fn_cq_set_internal／fn_cq_restore_internal,
--      與它們自己後半段同步草稿期的做法相同):
--      * issue_supervisor_certificate(監造確認單):auth.uid() 與監造身分在插入前已驗。
--      * revoke_inspection_confirmation(撤銷):同上。
--      * field_document_sign_inspection_form_internal(查驗表單簽署分支):簽署列 v_signed 已驗、p_uid 是簽署者本人。
--      旗標只包住那一句,同一交易其餘敘述(含 AFTER trigger 進來的 fn_cq_allocate_to_cap_internal)照舊各自開關;
--      set_config(…, true) 交易內有效,異常時隨子交易回滾,不會外洩到後續敘述。
--   3. work_item_pricing_basis_guard:INSERT／UPDATE／DELETE 一律要 fn_cq_internal()(DELETE 只放行工項／專案 cascade);
--      set_work_item_pricing_basis 在那一句 upsert 前後開關旗標。
--   4. valuations_guard:非登入者(not is_user)且沒有內部旗標時,INSERT 不得帶非草稿狀態;UPDATE 不得改 status、
--      invoice_date、paid_date、paid_amount,非草稿期別不得改 period_no／period_start／period_end。登入者的既有規則
--      逐字不變;內部重算路徑(security definer 且設旗標)不受影響。歷史遷移／pgTAP fixture 以 DBA 邊界(交易內
--      set local pmis.cq_internal='1')重現,與 P4e 對 valuation_items 的做法一致;e2e-real chain 11b 同步改為在旗標內核定。
--
-- 服務憑證拿不到旗標的原因:fn_cq_set_internal 雖可被 service_role 執行,但 PostgREST 每個 request 是獨立交易,
--   set_config(…, is_local=true) 只活在該交易;Edge 無法在同一交易內「先開旗標再寫表」。e2e-real chain 20 以真 PostgREST 釘住這一點。
--
-- 資料保留:不動任何一列。既有確認紀錄、撤銷紀錄、期別狀態全部不變。
-- 相容:前端只經 RPC(issue_supervisor_certificate／revoke_inspection_confirmation／sign_field_document／transition_valuation)
--   寫這兩張表,行為不變;Edge 本來就不寫。DBA／pgTAP 直接 insert 確認紀錄或非草稿期別的 fixture 需改在旗標內執行。
-- 回復:supabase/rollbacks/20260920230000_edge_credential_writer_seal.down.sql(七支函式各換回上一版定義、計價依據 trigger 回
--   before insert or update;沒有資料需要還原)。
-- pgTAP:edge_credential_writes.sql(service_role／superuser 直寫矩陣:確認量 insert／revoke、期別狀態 insert／update、計價依據
--   insert／update、RPC 路徑照常、旗標內的 DBA 邊界照常);confirmed_quantity_enforcement.sql／confirmed_quantity_concurrency.sql 的
--   直寫 fixture 改在旗標內;其餘直接建非草稿期別的 fixture(boq_reset_import、evidence_guards、formal_mode、p0_05_audit_events、
--   payment_flow、photos_storage、valuation_items_guard、confirmed_quantity_enforcement)同樣改在旗標內。

-- ═══════════════════════════════════════════════════════════════════════════
-- 1. inspection_confirmations_guard:INSERT 與 active→revoked 只認內部旗標(本體逐字沿用 20260920001500,只多兩段)
-- ═══════════════════════════════════════════════════════════════════════════
...
begin
  ...
  if tg_op = 'UPDATE' then
    ...
    if old.status = 'active' and new.status = 'revoked' then
      -- F2:撤銷只認內部旗標(revoke_inspection_confirmation 設);服務憑證／直連不得直接改寫
      if not public.fn_cq_internal() then
        perform public.fn_cq_raise_internal('VQ010', '撤銷監造確認只能由登入的監造經 revoke_inspection_confirmation 執行,不接受直接改寫(含服務憑證)');
      end if;
  ...
  -- INSERT
  -- F2:新確認只認內部旗標(issue_supervisor_certificate／查驗表單簽署設);服務憑證／直連不得憑空寫入。
  -- 放在所有內容檢查之前:不是簽發路徑就一律拒絕,不看內容是否「看起來合法」。
  if not public.fn_cq_internal() then
    perform public.fn_cq_raise_internal('VQ010', '監造確認只能由登入的監造親自簽發(監造確認單或查驗表單簽署)寫入,不接受直接寫入(含服務憑證)');
  end if;
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 2. issue_supervisor_certificate:簽發那一句 insert 開旗標(本體逐字沿用 20260919140000)
-- ═══════════════════════════════════════════════════════════════════════════
...
begin
  ...
  -- F2:guard 只認內部旗標;簽發者=已登入的本案監造(上面驗過),旗標只包住這一句 insert
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 3. revoke_inspection_confirmation:撤銷那一句 update 開旗標(本體逐字沿用 20260919140000)
-- ═══════════════════════════════════════════════════════════════════════════
...
begin
  ...
  -- F2:guard 只認內部旗標;撤銷者=已登入的本案監造(上面驗過),旗標只包住這一句 update
  v_prev := public.fn_cq_set_internal(true);
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 4. field_document_sign_inspection_form_internal:簽署寫確認量那一句 insert 開旗標(本體逐字沿用 20260920170000)
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 5. valuations_guard:非登入者的狀態／請款／撥款／非草稿期別欄位只認內部旗標(本體逐字沿用 20260919140000,只多兩段)
-- ═══════════════════════════════════════════════════════════════════════════
...
begin
  if tg_op = 'INSERT' then
    ...
    -- F2:非登入者(服務憑證／DBA 直連)也不得憑空建立非草稿期別;歷史遷移走內部旗標(DBA 邊界)
    if not is_user and not public.fn_cq_internal() and new.status <> '草稿' then
      perform public.fn_cq_raise_internal('VQ010', format('估驗期別只能以草稿建立;狀態 %s 只能由登入者經狀態機(transition_valuation)寫入,不接受服務憑證直接建立', new.status));
    end if;
    return new;
  end if;
  ...
  -- F2:非登入者(服務憑證／DBA 直連)的狀態轉移、請款／撥款欄位與非草稿期別欄位只認內部旗標——
  -- 狀態機與角色檢查只對登入者成立,沒有這一條,服務憑證可以略過「只有監造可核定／只有機關可登錄請款」直接改寫。
  if not is_user and not public.fn_cq_internal() and (
       new.status       is distinct from old.status
    or new.invoice_date is distinct from old.invoice_date
    or new.paid_date    is distinct from old.paid_date
    or new.paid_amount  is distinct from old.paid_amount
    or (old.status <> '草稿' and (
         new.period_no    is distinct from old.period_no
      or new.period_start is distinct from old.period_start
      or new.period_end   is distinct from old.period_end))) then
    perform public.fn_cq_raise_internal('VQ010', '估驗狀態、請款／撥款欄位與非草稿期別欄位只能由登入者經狀態機寫入,不接受服務憑證直接改寫');
  end if;
  return new;
end; $$;
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 6. work_item_pricing_basis:INSERT／UPDATE／DELETE 只認內部旗標;set_work_item_pricing_basis 的 upsert 開旗標(本體逐字沿用 20260919140000)
-- ═══════════════════════════════════════════════════════════════════════════
create or replace function public.work_item_pricing_basis_guard()
returns trigger language plpgsql security definer set search_path = public as $$
begin
  -- F2:計價依據決定可估驗上限的算法(inspection／supervisor_certificate／pro_rata／excluded),只認內部旗標
  -- (set_work_item_pricing_basis 設;登入的監造本人)。DELETE 只放行 cascade(工項或專案已不在)。
  if tg_op = 'DELETE' then
    if not exists (select 1 from public.work_items w where w.id = old.work_item_id)
       or not exists (select 1 from public.projects p where p.id = old.project_id) then
      return old;
    end if;
    if not public.fn_cq_internal() then
      perform public.fn_cq_raise_internal('VQ010', '計價依據只能由登入的監造經 set_work_item_pricing_basis 設定,不接受直接刪除(含服務憑證)');
    end if;
    return old;
  end if;
  if not public.fn_cq_internal() then
    perform public.fn_cq_raise_internal('VQ010', '計價依據只能由登入的監造經 set_work_item_pricing_basis 設定,不接受直接寫入(含服務憑證)');
  end if;
...
create trigger work_item_pricing_basis_guard before insert or update or delete on public.work_item_pricing_basis
...
begin
  ...
  -- F2:guard 只認內部旗標;設定者=已登入的本案監造(上面驗過),旗標只包住這一句 upsert
  v_prev := public.fn_cq_set_internal(true);
