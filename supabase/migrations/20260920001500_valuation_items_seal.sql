-- P4e｜收回估驗明細 valuation_items 的直接寫入(D-026 第 3 點;設計 docs/architecture/confirmed-quantity-valuation.md §9、§12、§19)。
--
-- 為什麼:P4b(20260919140000)把「每一單位本期增量都追得到監造確認來源」做成 DB 不變量,但刻意留了一個相容窗——
-- valuation_items 的直接 REST 寫入仍可寫草稿明細(標 backing='legacy'、送審／核定被檢查點擋)。P4c(PR #144)已移除
-- 全部客戶端寫入、P4d(PR #147)已上線撤銷／補證／調整,相容窗沒有任何合法使用者;它只剩「讓無依據數量出現在
-- 草稿與畫面上」的副作用。本支把明細收成「來源分配的投影」:只有 P4b 的重算路徑(同步確認量、設定累計量、
-- 確認量自動同步、補證、平台管理員維護調整——全部 security definer 且設 pmis.cq_internal)能寫。
--
-- 三層(比照 P1b 20260917210000 成本表做法,再加上 P4b 對新表一體適用的 guard):
--   1. 表級 GRANT:收回 PUBLIC／anon／authenticated 的 INSERT／UPDATE／DELETE(PostgREST 直接回 42501;anon 自
--      H2 20260919003000 起本來就沒有,這裡明示以免日後廣域 grant 帶回來)。SELECT 不動。
--   2. RLS:刪掉 `valuation_items_write`(for all)policy,只留 `valuation_items_select`——日後即使有人再下廣域 grant,
--      RLS 仍不放行寫入。
--   3. guard:valuation_items_guard 對「非重算路徑」的 INSERT／UPDATE／DELETE 一律 VQ010,所有寫入者一體適用
--      (service role 保有平台預設的表級 DML,和 valuation_item_sources／valuation_adjustments 一樣由 guard 擋;
--      DBA 直連也擋)。cascade 放行不變:期別被刪(草稿期刪除、專案刪除、標單重設)與工項被刪(單一工項刪除)時
--      父列已不在;已送審／核定／請款期別的明細仍不可被 cascade 刪(既有證據 guard 行為)。
--   原 guard 的兩條相容分支隨之移除:「非重算寫入標 backing='legacy'」(已沒有非重算寫入)與「已核定明細的備註
--   監造可改」(P4c 起前端沒有此入口;備註不是數量,但明細是投影,不再保留任何直接改寫)。
--
-- 順帶(P4d 20260919160000 已列):仍以 format('%s') 印 numeric(18,4) 的訊息改經 fn_cq_txt(60.0000 → 60),
--   create or replace 重建、簽章與 ACL 不變,本體除訊息數字外與 P4b 逐字相同:
--   fn_cq_item_state_internal(逐工項違反 message:over_contract／source_mismatch／legacy_source／cutoff／
--   batch_over_allocated)、inspection_confirmations_guard(減量訊息)、valuation_item_sources_guard(批次與截止日)、
--   fn_cq_reconcile_internal(recheck_note 的超出量)、admin_adjust_valuation_item(超過契約量)、valuation_items_guard。
--   同一行另修一處文字:單階段工項的批次確認被撤銷後,batch_over_allocated 訊息原本會印「缺必要查驗階段 (單階段)」
--   (缺階段清單的佔位字),改為只有多階段工項才說缺階段,單階段說「確認已撤銷或減量」;missing_stages 欄位值不變。
--   H3 允許清單不變(沒有新函式、沒有新 grant)。
--   未含:P3c 查驗表單簽署 field_document_sign_inspection_form_internal 的 6 處數量訊息與 inspections_defect_sync
--   的缺失說明(同一根因;P3e 並行重定義簽署路徑,避免互蓋,續接清單 §7 列為緊接項)。
--
-- 盤點(合併前):前端 src/ 無任何 valuation_items 寫入(P4c 起;billing.test.js 釘住);Edge 無估驗寫入(本 PR
--   新增 _shared/valuationWrites.scan.test.ts 釘住 supabase/functions 全部原始碼);RPC／trigger 的寫入全部經
--   fn_cq_set_internal(true)(fn_cq_recompute_item_internal、fn_cq_allocate_internal、sync_valuation_from_confirmations、
--   issue_supervisor_certificate 補證、admin_adjust_valuation_item);Demo 無 DB;seed.sql 不寫;pgTAP／真後端 fixture
--   的歷史資料改走 DBA 邊界(set local pmis.cq_internal='1' 並明示 backing='legacy')。
-- 資料保留:不刪表、不刪列、不改任何既有值;正式庫 26 列(已核定 5、草稿 21,其中 legacy 草稿數量由廠商「同步確認量」
--   以確認量為準)照舊可讀、照舊由檢查點擋。
-- 相容:舊版前端(P4c 前的快取分頁)的 upsert／insert 會收到 42501,friendlyError 顯示「操作未完成…(代碼 42501)」,
--   舊碼在寫入失敗時還原該格／刪掉剛建的期別,不會白畫面;重新整理即載入新版(HTML must-revalidate、無 service worker)。
-- 回復:supabase/rollbacks/20260920001500_valuation_items_seal.down.sql(重授權＋回復 for all policy＋六支函式回 P4b 版本)。
-- pgTAP:valuation_items_guard.sql(三角色＋平台管理員＋非成員＋service role＋DBA 直接寫入矩陣、RPC 路徑照常、
--   cascade 放行、核定凍結)、confirmed_quantity_enforcement.sql(訊息無 .0000;舊前端路徑改斷言 42501)。

-- ═══════════════════════════════════════════════════════════════════════════
-- 1. 表級 GRANT 與 RLS
-- ═══════════════════════════════════════════════════════════════════════════
revoke insert, update, delete on public.valuation_items from public, anon, authenticated;

drop policy if exists "valuation_items_write" on public.valuation_items;
-- valuation_items_select(成員可讀)不動

-- ═══════════════════════════════════════════════════════════════════════════
-- 2. valuation_items_guard:只放行重算路徑與 cascade
-- ═══════════════════════════════════════════════════════════════════════════
...
begin
  ...
  if not found then return coalesce(new, old); end if; -- 期別 cascade(專案刪除／草稿期刪除／標單重設)

  if tg_op = 'DELETE' then
    -- 工項 cascade(單一工項刪除)時工項列已不在;其餘直接刪除一律拒絕(重算路徑不刪明細,來源歸零即累計回前期)
    if not public.fn_cq_internal() and exists (select 1 from public.work_items where id = old.work_item_id) then
      perform public.fn_cq_raise_internal('VQ010', '估驗明細只能由確認量重算寫入,不可直接刪除;草稿期請按「同步確認量」或刪除整期');
    end if;
    -- 凍結類拒絕沿用 P0001(業務規則 raise;前端 friendlyError 原樣顯示,既有 pgTAP 斷言 P0001);
    -- 訊息改「估驗目前為「狀態」」:舊寫法「估驗已%」遇到「已核定」會印成「估驗已已核定」
    if v.status <> '草稿' then
      raise exception '估驗目前為「%」,明細不可刪除;請由監造退回後重編', v.status;
    end if;
    return old;
  end if;

  -- 明細是來源分配的投影:只有 P4b 重算路徑(security definer 並設 pmis.cq_internal)可寫;
  -- 所有寫入者一體適用(authenticated 另由表級 GRANT 與 RLS 收回;service role、DBA 直連一樣擋)
  if not public.fn_cq_internal() then
    perform public.fn_cq_raise_internal('VQ010', '估驗明細只能由確認量重算寫入(同步確認量／設定累計量),不可直接寫入');
  end if;
  ...
  if v.status <> '草稿' then
    if tg_op = 'INSERT' then
      raise exception '估驗目前為「%」,不可追加明細', v.status;
    end if;
    -- 已送審／核定／請款:數量、金額、備註永不改寫(含重算路徑);只有補證可把 backing 由 legacy 改為 confirmed
    if (to_jsonb(new) - 'backing') is distinct from (to_jsonb(old) - 'backing') then
      raise exception '估驗目前為「%」,明細數量與金額不可改寫;更正走估驗調整(撤銷確認→扣回)', v.status;
    end if;
    return new;
  end if;
  ...
  if new.cum_qty > qc then
    perform public.fn_cq_raise_internal('VQ005', format('累計量 %s 超過契約量 %s', public.fn_cq_txt(new.cum_qty), public.fn_cq_txt(qc)));
  end if;
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 3. 訊息數字改經 fn_cq_txt(本體與 P4b 20260919140000 逐字相同,只改 format 參數)
-- ═══════════════════════════════════════════════════════════════════════════
...
begin
  ...
  if p_valuation_id is not null then
    ...
    if st.cum_qty is not null then
      if st.cum_qty > st.contract_qty then
        vio := vio || jsonb_build_object('code', 'over_contract',
          'message', format('累計量 %s 超過契約量 %s', public.fn_cq_txt(st.cum_qty), public.fn_cq_txt(st.contract_qty)));
    ...
    if round(st.delta, 4) <> round(st.sources_sum, 4) then
      vio := vio || jsonb_build_object('code', 'source_mismatch',
        'message', format('本期增量 %s 與來源分配 %s 不符(缺監造確認來源)', public.fn_cq_txt(round(st.delta, 4)), public.fn_cq_txt(round(st.sources_sum, 4))),
    ...
    if st.legacy_qty <> 0 or exists (select 1 from public.valuation_item_sources
      ...
      vio := vio || jsonb_build_object('code', 'legacy_source',
        'message', format('數量 %s 來自歷史遷移,不是監造確認;需人工補證(監造確認單)', public.fn_cq_txt(st.legacy_qty)));
    end if;
    ...
    if v.status in ('草稿', '監造審核') then
      ...
      loop
        vio := vio || jsonb_build_object('code', 'cutoff',
          'message', format('批次「%s」截至 %s 的有效確認量 %s 少於累計分配 %s', r.batch_key, coalesce(v.period_end::text, '不設截止'), public.fn_cq_txt(r.eff), public.fn_cq_txt(r.alloc)),
  ...
  loop
    ...
    vio := vio || jsonb_build_object('code', 'batch_over_allocated',
      -- 單階段工項沒有「必要階段」可缺(fn_cq_missing_stages_internal 以「(單階段)」佔位):訊息改說確認已撤銷或減量
      'message', case when cardinality(st.required_stages) > 0 and cardinality(v_missing) > 0
                      then format('批次「%s」缺必要查驗階段 %s,有效確認量 %s 少於分配 %s', r.batch_key, array_to_string(v_missing, '、'), public.fn_cq_txt(r.effective_qty), public.fn_cq_txt(r.allocated_qty))
                      else format('批次「%s」有效確認量 %s 少於分配 %s(確認已撤銷或減量)', r.batch_key, public.fn_cq_txt(r.effective_qty), public.fn_cq_txt(r.allocated_qty)) end,
  ...
  if new.kind in ('confirmation', 'clawback') then
    ...
    for r in select * from public.fn_batch_allocation_check(confs, stages, w.unit, allocs) x
      ...
      perform public.fn_cq_raise_internal('VQ004',
        format('批次「%s」的有效確認量 %s 少於全部期別分配 %s(同一批次不可重複計價或缺必要階段)', r.batch_key, public.fn_cq_txt(r.effective_qty), public.fn_cq_txt(r.allocated_qty)),
    ...
    if new.kind = 'confirmation' then
      ...
      loop
        perform public.fn_cq_raise_internal('VQ004',
          format('批次「%s」截至 %s 的有效確認量 %s 少於累計分配 %s(截止日不符)', r.batch_key, coalesce(v.period_end::text, '不設截止'), public.fn_cq_txt(r.eff), public.fn_cq_txt(r.alloc)),
        ...
        update public.valuations
          set recheck_required = true,
              recheck_note = concat_ws(E'\n', recheck_note,
                format('工項 %s 批次「%s」超出有效確認量 %s(%s)', p_work_item, eff.batch_key, public.fn_cq_txt(excess), coalesce(p_reason, '')))
