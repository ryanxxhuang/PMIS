-- P0 恢復批4(P0-07):自 ba8a5ec 原樣取回(supabase/migrations/20260710000600),重新編號部署。
-- 修改(相對原版):requirements_snapshot_guard 注入「專案刪除 cascade 放行」(理由同 P0-06)。
-- 依賴前移:can_review_requirement 原定義於已取消的批3(party-based has_project_authority)。
-- 依「全域 org_type 模型定案」改寫:需求審核=機關(owner)或監造(supervisor)。
-- 刻意不給專案管理者例外——「技術管理者≠契約審核權」是 P0 的核心完整性語義
-- (hotfix 文件明載),單人試用的廠商建立者不能自審自己的履約需求。
create or replace function public.can_review_requirement(p uuid)
returns boolean language sql security definer stable set search_path = public as $$
  select p in (select public.my_project_ids())
     and public.my_org_type() in ('owner','supervisor')
$$;
revoke all on function public.can_review_requirement(uuid) from public, anon;
grant execute on function public.can_review_requirement(uuid) to authenticated;

-- 依賴前移:批3曾把 requirements_update 收成 party 審核制;取消批3後改為
-- 「專案成員皆可達 guard」——真正的生命週期/欄位保護由 requirements_snapshot_guard
-- 全權強制(機關的直接 PATCH 會被 guard 明確拒絕,而非 RLS 靜默吞掉)。
...
-- 依賴前移:引註凍結 guard 原屬已取消的批3,但為權限模型無關的契約完整性規則
-- (已審查 Requirement 的證據引註必須凍結),隨 P0-07 一併恢復。
...
declare
  ...
  run_status text;
begin
  -- [批4加固] 專案刪除 cascade 放行(父專案列已先刪);直接刪除仍受下方原始規則管制。
