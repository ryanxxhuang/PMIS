...
export const QUERY_TOOLS: ToolDef[] = [
  ...
  },
  {
    ...
    description:
      '列出「球在我方(使用者所屬單位)」的待辦:未結案缺失、待審送審件、未結案 RFI、待處理估驗、待查驗、待審查／核定的變更設計、待處理觀察、' +
      '現場文書(待簽署／待補欄位／待提送／待收件／被退回)、逾期契約期限;另附 setup_pending=設定沒補齊的契約期限(責任方／時點／基準日／循環規則／停止條件沒設或回填待核對;不歸任何一方,三方都看得到)。' +
...
// 會寫的工具,但只寫現場文書草稿(該日施工日誌文件的 AI 版本,P6b-2 起與照片起稿同一條路)與 agent_actions——
// 不寫 daily_logs(事實表只由使用者簽署落庫)。
export const DRAFT_DAILY_LOG_TOOL: ToolDef = {
  name: 'draft_daily_log',
  description:
    '當使用者要你幫忙寫/擬今天(或某天)的施工日誌時呼叫。會讀當日已上傳的現場照片、' +
    '昨日日誌與天氣,建立(或更新)該日施工日誌的文件草稿,並放進使用者的草稿收件匣。你不會建立正式日誌,' +
    '使用者必須親自填寫各工項數量、審核並簽署後才成立;照片看不出的數量一律留空。',
  input_schema: {
    type: 'object',
    properties: {
...
export const DRAFT_INSPECTION_TOOL: ToolDef = {
  ...
  description:
    '當使用者說要做/擬自主檢查表或查驗紀錄時呼叫。會依工項挑出合適的自主檢查表' +
    '範本,把日期、部位、工項與當日照片帶好,建立一份自主檢查表文件草稿並放進草稿收件匣(簽署前不是正式紀錄)。' +
