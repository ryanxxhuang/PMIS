...
import { MODELS, jsonResponse as json, dbErrorResponse } from '../_shared/claude.ts'
...
Deno.serve(aiJsonHandler({
  feature: 'documents.classify',
  build: async ({ body, gate }) => {
    const versionId = body.document_version_id
    if (!isUuid(versionId)) return json({ error: '缺少有效的 document_version_id' }, 400)
    ...
    const { data: version, error: versionError } = await gate.userClient
      ...
      .eq('id', versionId)
      .maybeSingle()
    if (versionError) return dbErrorResponse('classify-document.version', versionError)
    ...
    const { data: pages, error: pagesError } = await gate.userClient
      ...
      .order('page_number')
      .limit(SAMPLE_PAGES)
    if (pagesError) return dbErrorResponse('classify-document.pages', pagesError)
    const sample = (pages ?? [])
