...
insert into public.project_members (project_id, user_id, role) values
  ('e7100000-0000-0000-0000-00000000000a', 'e7000000-0000-0000-0000-000000000001', 'member'),
  ('e7100000-0000-0000-0000-00000000000b', 'e7000000-0000-0000-0000-000000000001', 'admin'),
  ('e7100000-0000-0000-0000-00000000000a', 'e7000000-0000-0000-0000-000000000002', 'admin'),
  ('e7100000-0000-0000-0000-00000000000a', 'e7000000-0000-0000-0000-000000000003', 'member');
...
-- 批3取消後:直接 PATCH 不再被 RLS 靜默,而是被 lifecycle guard 明確拒絕(等效防護)。
select throws_ok($$
  update public.requirements set status = 'approved'
  where id = 'e7700000-0000-0000-0000-000000000004'
$$, 'P0001', null, 'contractor direct PATCH is rejected by the lifecycle guard');
reset role;
select public.pmis_p07_login(null);
...
select public.pmis_p07_login('e7000000-0000-0000-0000-000000000001');
set local role authenticated;
-- [批3已取消] 契約禁止兼職→同一人跨案不同角色的情境不存在;全域 org_type 模型下
-- 監造 org 的成員在其所屬專案皆可審核。原斷言保留於 git 歷史。
