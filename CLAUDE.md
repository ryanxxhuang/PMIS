...
- **W5-2～W5-4 本機實作與統一收尾核對已完成，不要重做**：Requirement 單向 migration／rollback、legacy caller 退場、成員模型防誤用，以及試體缺失 Demo／DB 漂移修正均已完成；尚未 commit、PR 或部署。下一動作須等使用者明確授權後提交 W5，此前不要開始 W6。
