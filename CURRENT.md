...
- UI 採 [Apple 規範](docs/UIUX-Apple-設計規範.md)，token 在 `src/index.css`，圖示 `lucide-react`。`listDetail.jsx` 共用清單／詳情殼已供契約兩頁、工安頁與品質頁（查驗／試體／觀察分段）使用；列印用 `.paper` 與 `PrintToolbar`。
