...
> CURRENT｜2026-09-17｜三方操作流程優化＋D-026 P1b 退場頁唯讀化＋P4a 純計算層＋P2a 現場文書資料層＋P2b Edge 起稿＋P2d 施工日誌存版／簽署／提送 RPC；正式發布狀態見 §6.3。
> 包含契約整理、Apple UI、D-022、工安清單／詳情殼，以及全案程式與文件整理；驗證見 [BASELINE](docs/BASELINE.md)。前端已由 main 自動部署；DB 已同步至 `20260917213900`、Edge 最後重佈 2026-09-17（全部 18 支），版本與核對範圍見 §6.3。
> 進行中工作（D-026 產品瘦身與四類文書、監造確認量估驗聯動）見 [續接清單](docs/reviews/2026-09-17-product-slimming-worklog.md)；本檔只記已上線現況：該包目前上線的是 P1a 導覽重組、P1b 退場頁唯讀化（成本寫入由 DB 收回、跨案選案清單、稽核檢核移入估驗頁、今日工作單一頁名；前端＋migration，demo 站已同步重佈）、P1c／P1d 文案對齊與抽屜斷點修正（前端）、P4a 純函式 migration（尚無呼叫端）、P2a 現場文書資料層 migration（表、RLS、guard、稽核）、P2b Edge `draft-field-documents`（照片批次辨識、候選文書、施工日誌 AI 版本；尚無前端呼叫端，P2c 才接）與 P2d 存版／簽署／提送 RPC 與 `daily_logs` guard migration（只有 `daily_log` 簽署分支；尚無 Edge／前端呼叫端），其餘仍是設計文件。
...
- **DB**：2026-09-17 `supabase db push` 先後套用 `20260917120000_confirmed_quantity_calc`（P4a 純函式）、`20260917201000_field_documents`（P2a 現場文書資料層）、`20260917205000_field_document_rpcs`（P2d，由該單元套用）、`20260917210000_cost_items_retire`（P1b 成本寫入收回）、`20260917213500_ai_field_docs_draft`（P2b AI 功能 seed）與 `20260917213900_api_roles_table_ddl_privileges`（H1 API 角色表級 DDL／維護類權限收回＋default privileges）；`migration list --linked` 核對本地與遠端 66 筆全部對齊，最新 `20260917213900`。前一次 2026-09-11 套用 `20260911100000_demo_requests_revoke_grants`、`20260911100100_contract_parse_retire`、`20260911110000_project_admin_single_source`。
- **Edge**：2026-09-17 以 `--use-api` 從 main（`b52cd25`，P2b）重佈全部 18 支；`functions list` 核對既有 17 支版本均 +1（agent-run 15、extract-requirements 14、send-reminders 17 且 `verify_jwt=false`、classify-document 4 等），新增 `draft-field-documents` 版本 1。前一次 2026-09-11 從 main 重佈 17 支。線上另有 `demo-request` 一支由行銷站 repo 部署，不在本 repo。
