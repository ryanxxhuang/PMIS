...
- **2026-09-17 瘦身 P1c＋P1d 文案對齊四主入口與手機抽屜斷點缺陷（D-026）**：PR #108（分支 `codex/slimming-p1cd`，commit `2126496`；merge commit 與合併後 `check:prod` 於下一個單元的文件同步補記）。PR 內容為純前端文案與 Layout 抽屜狀態推導＋文件，不含 DB migration 或 Edge 部署；本機驗證 120 檔 1,259 項單元、4 支 35 項 Demo E2E（含新增抽屜斷點測試）、lint／build／check:docs 通過。demo 站未重佈（待 P1b 後一次重佈）；正式站登入後流程與真人驗收未核對。
