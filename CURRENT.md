...
> CURRENT｜2026-09-11｜工作分支 `refactor/product-wide`；現況依本分支程式核對。
> 既有契約整理、Apple UI、全案重構、D-022 與工安清單／詳情殼已提交；本輪進度共用化與文件精簡見 [ROADMAP](docs/ROADMAP.md)。分支尚未合併 main、未部署。線上資訊只代表 §6.3 最後一次核對。
...
當前結果與指令只見 [BASELINE](docs/BASELINE.md)。既有 pgTAP 曾在本機從零套用 migrations 後全過；本輪未動 DB。真後端 E2E 本輪未跑（先前 chain3 模型金鑰失效）；Deno 型別檢查未完成。抽取準確率／召回率未用真契約量測；`completed` 不代表語意正確。真人手機輪與真案三角色實機驗收仍待完成。
...
- **Edge**：未逐支核對線上版本；最後重佈紀錄是 PR #48／#51。分支 `_shared/` 重構尚未部署；部署時需將 17 支 Edge 一併重佈與核對版本。
- **前端**：最後已知線上版本為 main PR #62；2026-09-07 首頁 HEAD 200、七項安全標頭齊全。這不代表本輪重構已上線或登入後流程已驗證。
- **舊站**：`pmis.pages.dev` 與 `ryanxxhuang.github.io/PMIS` 在 2026-09-07 仍可公開存取，待處理。
- 部署依 [runbook](docs/operations/deploy.md) 執行；套用後在本節記日期、migration／Edge 版本與驗證。本輪只 commit／push 分支，未合併 main、未部署、未套正式 migration。

### 6.4 本分支成果

契約整理補頁面完整性、核對例外篩選、載入錯誤與切案防護；Apple token／圖示／字級與收件匣已落地；巨型頁面與 Edge 拆分、錯誤遮罩／AI 骨架共用、日期／金額共用化、lint／測試補強、D-022 及工安清單／詳情殼已提交。細節從 Git commit／PR 追溯，不再重複抄進多份文件。
