# PMIS — 現況與路線圖（2026-07-09 歷史快照）

> **文件狀態：HISTORICAL。** 本文保留 2026-07-09 當時的 PMIS 現況、缺口與路線判斷，
> 不再是目前系統或後續路線的權威來源。文內「唯一現況文件」、功能數量、測試數量、
> GitHub Pages、未實作項目等敘述均應按當時快照閱讀，不逐段回寫。
>
> 目前產品與系統真相見 [CURRENT.md](CURRENT.md)；長期方向見
> [GovAgent 北極星](docs/北極星-政府機關-Agent-平台.md)；文件分類見
> [docs/README.md](docs/README.md)。
>
> 以下為原始快照內容。
>
> **GTM 轉向(2026-07-09):機關優先**——第一個付費客戶高機率是機關,機關端功能已補齊:
> 跨案總覽(/portfolio,機關預設落地頁)+驗收/結算(/acceptance,法定期限自動起算)。
> 監造端結構關鍵 **ITP 檢驗停留點(/itp,W/H/R)** 已落地(原附錄 A 規劃)。
> 伺服器端 RBAC 已上線(RLS 分角色+8 個狀態轉移 trigger,三方可安全共用專案)。
> store.jsx 已拆分為領域 slices(缺口 #5 已解)。AI 全跑 Claude API。測試 108 綠。
> 網域購買擱置(專案名稱未定)→ Resend 網域驗證連帶暫停。
